...
- Empty state with undo: showMedia() checks moveHistory.length when mediaFiles.length===0 — shows showEmptyStateWithUndo() (preserves undo history, no drop zone) vs showDropZone() (genuine empty, no history); applies to both single and compare mode transitions
...
- Toggle bug fix (TASK-022 design): showCompareMedia() and moveComparePair() use switchToSingleModeUI() (non-toggling) instead of toggleViewMode() to avoid infinite loop when <2 files remain
...
- TASK-022 design spec approved (commit ab73212): Fix compare mode last-pair error cascade — two bugs: (1) infinite loop from isCompareMode=false + toggleViewMode() toggling it back to true; (2) drop zone shown when moveHistory still has entries; fix: new switchToSingleModeUI() helper (non-toggling, extracted from toggleViewMode()), new showEmptyStateWithUndo() method, moveComparePair() early exit when <2 files remain (resets state flags, clears pending ML state, calls switchToSingleModeUI()), showMedia() checks moveHistory.length before choosing showEmptyStateWithUndo() vs showDropZone(), handleCancel() detects compareMode:true tag on history entries to restore both compare-pair files in one undo

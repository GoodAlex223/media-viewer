...
- TASK-020 completed (commits 4e3171e–993d25d): ML sorting race condition fix fully implemented — defers showMedia() in moveComparePair() when ML-sorted compare mode; bypasses 100ms re-score debounce in updateComplete/reverseUpdateComplete when pendingCompareUpdates hits 0; adds mediaNavigationInProgress guard to all four compare rating handlers (handleLeftLike/handleLeftDislike/handleRightLike/handleRightDislike); adds orphan-timeout clearTimeout before setting new pendingCompareTimeout; score delta notification "ML updated: N files rescored (X↑ Y↓)" with >0.05 threshold fires in scoreComplete; fallback 3s timeout unblocks UI if scoreComplete never arrives
- TASK-020 implementation plan added (commit 4c2e5c6): 8-task plan for ML sorting race condition fix — adds pendingCompareRefresh/pendingCompareUpdates/pendingCompareTimeout/previousScores state; defers showMedia() in moveComparePair() when ML-sorted; bypasses 100ms re-score debounce in updateComplete when counter hits 0; adds score delta notification in scoreComplete; creates tests/ml-pair-selection.test.js (7 unit tests for selectMlPair() algorithm); adds 5 BACKLOG items; plan at docs/superpowers/plans/2026-03-21-task-020-ml-sorting-fix.md
- TASK-020 design spec added and revised (commits 16110c1 + 1fac072, 8 spec review fixes applied): ML sorting pair ordering investigation — root cause analysis of 6 user-reported issues; Issue 2 confirmed as race condition bug: moveComparePair() calls showCompareMedia() before updateMlModelWithFeatures() 100ms debounce re-score completes, next pair renders with stale scores; fix uses pendingCompareRefresh flag + pendingCompareUpdates counter (2 for rating, 1 for undo) to bypass debounce and defer showMedia() until scoreComplete arrives; fallback timeout (3s) unblocks UI if scoreComplete never arrives; mediaNavigationInProgress stays true during pending window; score delta UX notification "ML updated: N files rescored (X↑ Y↓)" via showNotification(); unit test file tests/ml-pair-selection.test.js planned (7 test cases for pair selection logic); Issues 1, 4, 6 working as designed; 5 BACKLOG items: content-understanding features, auto re-sort after N ratings, model diagnostics panel, margin-based pairing, score confidence indicator

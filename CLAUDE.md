...
- Settings panel (F1) with number inputs/checkboxes wired to localStorage; constructor validates/clamps saved values; `enableClipFeatures` key (default true) toggles CLIP semantic embedding extraction, wired to `#clipFeaturesToggle` checkbox (~87 MB model download on first use); on disable: async handler also calls `deleteSortCache('clip')` and reverts `sortAlgorithm` to `'vptree'` if it was `'clip'` (prevents stale cache + broken-sort-click UX)
...
**Next planned**: `feature/clip-sort-followups` branch active — spec at `docs/superpowers/specs/2026-05-02-clip-sort-followups-design.md`; Tasks 1+2 done (export extension 91f87e6, 4 `sortMediaBySimilarityClip` characterization tests → 164/164); remaining: algorithm-aware `insertNewFilesInSortedOrder` (CLIP vs Hamming branch), CLIP toggle-off cache+state cleanup, regression tests for new insertion logic. Source: BACKLOG lines 60-71 (Group D follow-ups).
...
- Sort cache new-file insertion: `insertNewFilesInSortedOrder(sortedFiles, newFiles, algorithm)` — `algorithm` comes from `cachedData.algorithm` (written at save time); `'clip'` path uses cosine distance over `clipCache`, other values (`'vptree'`, `'mst'`, `'simple'`, undefined) use Hamming; end-append fallback for missing vectors in both paths; stale caches without `algorithm` field fall through to Hamming (safe default)
...
- `insertNewFilesInSortedOrder` is algorithm-aware (post feature/clip-sort-followups): takes a third `algorithm` param from `cachedData.algorithm`; `'clip'` branch scores by cosine distance over `clipCache`, all other values use Hamming; no on-demand CLIP extraction inside this function — missing-vector files are end-appended (cache-hit path must be near-instant); old caches without `algorithm` field default to Hamming via the `else` branch
- CLIP toggle-off cleanup (post feature/clip-sort-followups): the `#clipFeaturesToggle` `change` handler is `async`; on disable it `await this.deleteSortCache('clip')` (clears stale `'clip'` key from `.sort_cache.json`) then reverts `this.sortAlgorithm` to `'vptree'` and updates the dropdown — prevents "CLIP disabled" error on next sort click; no inverse op on toggle-on (first sort rebuilds from live `clipCache`); revert target is always `'vptree'` (constructor default), not `'mst'` or `'simple'`

...
- Training-vector cache (`ml-training.js`, `MlTrainingManager`): each of the like/dislike folders gets its own `.feature_cache.json` — same v4 binary format, `size`/`mtime` staleness rule, streaming IO and atomic write as the source folder's, serialized through the same `_acquireCacheIoLock` host mutex (one lock, one writer, across source and training folders alike). **Isolation is load-bearing**: entries live only in a `Map` local to the call (`_loadVectorCache`/`_saveVectorCache`) and are never merged into `this.featureCache`/`this.clipCache`/`this.featureMetadata` — those belong to the source folder, and its own 30s auto-save + load-time prune would otherwise churn like-folder entries into the wrong file. Each training folder's shrink-guard baseline (`diskCount`) is independent; poisoned/unresolvable entries (`size`/`mtime` 0) are skipped, never written.

...
**Next planned**: `feature/clip-sort-followups` complete — all BACKLOG Group D follow-ups done: export extension (91f87e6), `sortMediaBySimilarityClip` characterization tests (30486df), `calculateCosineDistance` added to `MediaViewer` (cdf631e), algorithm-aware `insertNewFilesInSortedOrder` CLIP+Hamming branch (2252d32), `extractAsyncMethod` + insertion regression tests (ae1f241), `applyCachedSortOrder` algorithm-pass fix (0eaf7ca), CLIP toggle-off sort-cache cleanup + `sortAlgorithm` revert to `'vptree'` (0ce9cec), polish: cosine null-return doc + toggle-off reorder (80ac67d). Branch ready to merge/PR. Pull next batch from `docs/planning/BACKLOG.md`.

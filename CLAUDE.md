...
- TASK-024 in progress (commit de8ccc4): Per-folder feature cache — load-folder IPC response now includes `mtimeMs` (from existing `fs.stat()` call) for each media file object alongside name/path/size/type; enables renderer to use file modification timestamps for cache invalidation; design: cache schema v3 — per-entry format changes from bare array to `{vector, size, mtime}` object; `featureMetadata` Map (`filePath → {size, mtime}`) tracks metadata in MediaViewer constructor; `loadFeatureCache()` moved out of lazy-init guard to reload on folder switch; progress indicator to show cached count ("Extracting features: N/T new (M cached)"); remaining changes in media-viewer.js (~60-80 lines, 7 locations)

...
- TASK-025 completed (commits 6019189–8fd9934): Application logging to file with auto-cleanup — new `logger.js` CommonJS module (`init/log/warn/error/cleanup/getLogPath`); `fs.writeSync` writes `[YYYY-MM-DD HH:MM:SS.mmm] [LEVEL] [source] msg\n` to `app.getPath('logs')/media-viewer.log` (overwritten on startup); `main.js` intercepts `console.log/warn/error` after `logger.init()` via `formatArgs()` helper that preserves Error stack traces (`a instanceof Error ? a.stack : String(a)`); new fire-and-forget `ipcMain.on('log-renderer-error')` handler routes renderer errors to `logger.warn/error`; `logger.cleanup()` called in `will-quit` (deletes log on clean exit, crash logs survive); `preload.js` exposes `logError: (data) => ipcRenderer.send('log-renderer-error', data)` (never blocks renderer); `media-viewer.js` adds `logError` call in `showError()` + `window.onerror` and `unhandledrejection` global handlers; `logger.js` added to ESLint block 1; `tests/logger.test.js` unit tests (12 tests); code review fix (8fd9934): `formatArgs` replaces `args.join(' ')` to preserve stack traces; design spec indexed in docs/README.md

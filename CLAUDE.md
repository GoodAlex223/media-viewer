...
- TASK-022 design spec approved + implementation plan added (commits ab73212 + 851c554): Fix compare mode last-pair error cascade — two bugs: (1) infinite loop from isCompareMode=false + toggleViewMode() toggling it back to true; (2) drop zone shown when moveHistory still has entries; fix: new switchToSingleModeUI() helper (non-toggling, extracted from toggleViewMode()), new showEmptyStateWithUndo() method, moveComparePair() early exit when <2 files remain (resets state flags, clears pending ML state, removes stale leftMediaWrapper/rightMediaWrapper DOM, calls switchToSingleModeUI()), showMedia() checks moveHistory.length before choosing showEmptyStateWithUndo() vs showDropZone(), handleCancel() detects compareMode:true tag on history entries to restore both compare-pair files in one undo; plan at docs/superpowers/plans/2026-03-22-task-022-fix-compare-last-pair.md (8 tasks: switchToSingleModeUI, showEmptyStateWithUndo, moveComparePair fix, showCompareMedia guard fix x2, showMedia fix, compare-pair undo, E2E test)
- TASK-022 implementation step 1 (commit 960d2fc): Added `switchToSingleModeUI()` method to MediaViewer (media-viewer.js line 3365, +19 lines) — non-toggling helper that updates DOM to single-mode state (sets isCompareMode=false, updates viewModeLabel, shows/hides controls, hides left/right file info panels, calls hidePredictionBadges() + closeAllZoomPopovers()) without triggering the full media cleanup + toggle cycle of toggleViewMode(); used by graceful-fallback paths that must enter single mode without risk of toggling back to compare mode
- TASK-022 implementation step 2 (commit 69210c7): Added `showEmptyStateWithUndo()` method to MediaViewer (+9 lines) — cleans up current media if present, hides loading spinner, updates folder info and navigation info; leaves container empty and toolbar functional (Ctrl+Z remains accessible) instead of replacing UI with the drop zone; called by showMedia() when mediaFiles.length===0 and moveHistory.length>0, and directly by moveComparePair() when last pair is rated

...
- `insertNewFilesInSortedOrder` per-file abort check (resolved 2026-05-21): both CLIP and hash branches now start their outer `for` loop with `if (this.sortAbortController?.signal.aborted) throw new Error('Sort aborted');` (media-viewer.js outer loops at L~5125 CLIP, L~5174 hash); outer-only, not inner hot path; optional chaining safe because `sortAbortController` is null between sorts; error message matches existing pattern at L4223/4254; event-loop yielding (`await new Promise(r => setTimeout(r, 0))`) deferred to BACKLOG pending observed UI freeze.

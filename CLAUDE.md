...
- TASK-024 design approved (commit 5ceabc2): Per-folder feature cache fix — root cause: `loadFeatureCache()` inside lazy-init guard `if (!this.mlWorker || this.featureWorkers.length === 0)` (media-viewer.js); workers survive folder switches so guard is skipped on second+ folder, `loadFolder()` clears `featureCache` but cache is never reloaded; Approach A (minimal fix) chosen: (1) move `loadFeatureCache()` out of lazy-init guard — call unconditionally in `handleSortByPrediction()` before `startBackgroundFeatureExtraction()`; (2) cache schema v3 — per-entry format changes from bare array to `{vector, size, mtime}` object; `mtimeMs` added to `load-folder` IPC response; `featureMetadata` Map (`filePath → {size, mtime}`) populated at extraction + load time; `saveFeatureCache()` reads `featureMetadata` to write v3; in-memory `featureCache` still stores `Float32Array` (consumers unchanged); deleted file pruning implicit (entries absent from `mediaFiles` not loaded, disappear on next save); (3) progress indicator shows cached count — "Extracting features: N new (M cached)" and "Feature extraction complete — N files (M cached, K extracted in Xs)"; files to change: main.js + media-viewer.js only

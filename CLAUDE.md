...
- TASK-024 in progress (commits de8ccc4–fd8235c + Task 6): Per-folder feature cache — cache schema v3 (per-entry `{vector, size, mtime}`, FEATURE_CACHE_VERSION 2→3); `featureMetadata` Map (`filePath → {size, mtime}`) populated at all 6 `featureCache.set()` call sites (worker result, compare mode fallback left/right, single-mode rating, computeFeatures canvas path, priority extraction); `loadFeatureCache()` moved out of lazy-init guard (Task 6 core bug fix — workers survive folder switch so lazy-init block skipped on 2nd+ folder, featureCache never reloaded from disk); remaining: Task 7 progress indicators with cache hit count

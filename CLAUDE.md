...
- TASK-023 fix (commit 9f777c5): Scoped `lucide.createIcons()` calls — replaced `nodes: [element]` param with `root: element` param in all three call sites (modal, zoom popover, compare pane wrappers); root cause: passing an array to `nodes=` caused the library to re-replace already-processed global icons, producing stale icon refs; compare mode split into two separate `createIcons({ root })` calls (one per pane wrapper) instead of one call with an array

...
- TASK-024 design approved (commits 5ceabc2 + 84526ee): Per-folder feature cache fix — root cause: `loadFeatureCache()` inside lazy-init guard `if (!this.mlWorker || this.featureWorkers.length === 0)` (media-viewer.js); workers survive folder switches so guard is skipped on second+ folder, `loadFolder()` clears `featureCache` but cache is never reloaded; Approach A (minimal fix) chosen: (1) move `loadFeatureCache()` out of lazy-init guard — call unconditionally in `handleSortByPrediction()` before `startBackgroundFeatureExtraction()`; (2) cache schema v3 — per-entry format changes from bare array to `{vector, size, mtime}` object; `FEATURE_CACHE_VERSION` bumped 2→3 (existing v2 caches auto-invalidated on first load); `mtimeMs` added to `load-folder` IPC response (1 line in main.js, from existing `fs.stat()` call); `featureMetadata` Map (`filePath → {size, mtime}`) added to MediaViewer constructor state, populated at extraction time and cache load time, read by `saveFeatureCache()` to write v3 entries — avoids depending on `this.mediaFiles` at save time (files may have been moved/rated since extraction); in-memory `featureCache` still stores `Float32Array` (consumers unchanged); on-disk `{vector, size, mtime}` is serialization format only; deleted file pruning implicit (entries absent from `mediaFiles` not loaded into memory, disappear on next save); (3) progress indicator shows cached count — "Extracting features: N/T new (M cached)" and "Feature extraction complete — N files (M cached, K extracted in Xs)"; "All N features loaded from cache" when 0 new files; files to change: main.js (1 line) + media-viewer.js (~60-80 lines, 7 locations)

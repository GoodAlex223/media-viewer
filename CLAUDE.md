...
- `kickoffBackgroundExtractionIfEnabled` tests (6 cases, all in `describe('kickoffBackgroundExtractionIfEnabled', …)`): (1) CLIP-disabled no-op — `enableClipFeatures: false`, asserts none of `initializeFeaturePool`/`initClipModel`/`startBackgroundFeatureExtraction` called; (2) fresh-state full-init — empty `featureWorkers: []`, asserts all three called once each; (3) workers-exist skip-pool — `featureWorkers: [{}]`, asserts `initializeFeaturePool` NOT called, `initClipModel` + `startBackgroundFeatureExtraction` each called once; (4) CLIP-already-ready skip-initClipModel — `clipWorkerReady: true`, asserts `initClipModel` NOT called, `startBackgroundFeatureExtraction` called once; (5) download-in-progress skip-initClipModel — `clipModelDownloading: true`, same assertions as (4); (6) extraction-rejects — `startBackgroundFeatureExtraction` returns rejected Promise, asserts `window.electronAPI.logError` called with message containing 'boom'; uses `extractMethod()` (sync, not `extractAsyncMethod`); `globalThis.window` patched in `beforeEach`/`afterEach` to `{ electronAPI: { logError: vi.fn() } }`; `makeCtx` defaults: `featureWorkers: [], clipWorkerReady: false, clipModelDownloading: false, initializeFeaturePool/initClipModel/startBackgroundFeatureExtraction` as `vi.fn()`
...
- `kickoffBackgroundExtractionIfEnabled()` (fixed 091fa55/be50953, was HIGH-priority silent-failure bug): guards on `!this.enableClipFeatures` (early return); skips `initializeFeaturePool()` when `featureWorkers.length > 0` (be50953 guard — double-init causes duplicate workers); skips `initClipModel()` when `this.clipWorkerReady` is already true; always calls `this.startBackgroundFeatureExtraction()`; called from `loadFolder()` to ensure CLIP extraction fires on every folder open regardless of sort path

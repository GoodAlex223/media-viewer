...
- `extractAsyncMethod(methodName)` helper: mirrors `extractMethod` but uses `Object.getPrototypeOf(async function(){}).constructor` (AsyncFunction) so `await` inside the method body is valid; regex matches `async methodName(params) {`; used in `tests/media-viewer-utils.test.js` for testing `insertNewFilesInSortedOrder` CLIP path
...
- `insertNewFilesInSortedOrder` is algorithm-aware: takes a third `algorithm` param; caller is `applyCachedSortOrder()` which passes `cachedData.algorithm` (written at sort-save time — source of truth for what distance metric the cache represents); `'clip'` branch scores by cosine distance over `clipCache`, all other values (`'vptree'`, `'mst'`, `'simple'`, undefined) use Hamming; no on-demand CLIP extraction inside this function — missing-vector files are end-appended (cache-hit path must be near-instant); omitting the `algorithm` arg silently falls through to Hamming regardless of cache type — this was the 0eaf7ca bug
...
- CLIP background extraction may silently not fire on folder load (pre-existing bug, found 2026-05-03): repro — enable CLIP (default ON), open a fresh folder with no prior `.feature_cache.json`, wait 60+ seconds; observed: no cache file created, no progress notification, no CLIP-model-load notification, no console errors; CLIP sort then throws `"Only 0 files have CLIP embeddings."` from `handleSortBySimilarity`; hash sorts (MST/VPTree) work normally on the same folder; investigation entry point: trace `loadFolder` → `startBackgroundFeatureExtraction` → CLIP queue path and add diagnostic logging at each guard; affected files: `media-viewer.js` (extraction orchestration), possibly `main.js` (CLIP IPC); NOT introduced by feature/clip-sort-followups (branch diffs are all downstream of having CLIP vectors)

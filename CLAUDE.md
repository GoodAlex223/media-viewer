...
- `predictionScores` sync: `applyPredictionSortResult` (called from the `sortComplete` handler) writes `message.scores` (filename→score) into `predictionScores` before reassigning `mediaFiles`, so badges match the sorted order (guarded `if (message.scores)`).
...
- `handleTournamentUndo` has **two paths**: default `engine.undo()` (O(1) inverse-delta since CW-T) vs the special-move branch (`lastMove.actionType === 'special'`). `engine.removeFile` is NOT tracked by `engine.undo()`, so the special path manually restores the file on disk (`moveFile` IPC), re-adds it to `mediaFiles` + `engine.files`, calls `restoreFeatureCachesFromHistory`, re-persists (`_schedulePersist`), re-renders — and on error pushes `lastMove` back onto `moveHistory`.
- `engine.files` vs `strategy.files` **diverge after `removeFile()`** — `engine.files` is authoritative for `getTierBreakdown()`/`handleApply()`. Undo entries carry `filesSnapshot` (recorded via `removeFile(file, {trackUndo:true})` ONLY at the renderer's `-1` auto-prune site for an externally-missing file, reversed by `engine.undo()`) so tier counts survive a removeFile→undo; the O(1) inverse-delta of prior picks + `strategy.serialize()` alone cannot resurrect a removed file's strategy state. The special-move removal is deliberately left `trackUndo:false` so the renderer special-undo stack and the `engine.undo()` stack don't desync.

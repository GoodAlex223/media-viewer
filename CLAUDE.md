...
- `mlWorker` is **lazy** (`initializeMlWorker()` runs on the first AI sort / settings toggle) — an E2E that needs real scoring must call it, override `getCombinedFeatures` with **576-dim** vectors, and warm the model (≥3 likes + ≥3 dislikes) or `scoreAll` replies `scores: null` and a deferred window waits out its 3 s fallback. Forcing `isSortedByPrediction = true` alone leaves `mlWorker` null, so `postedUpdates` is 0 and the deferred-refresh protocol is never exercised (see `compare-mode.test.js` "REAL ML worker").
...
- ML compare refresh: `pendingCompareRefresh`/`pendingCompareUpdates` defer `showMedia()` until re-scoring completes (3s fallback); `mediaNavigationInProgress` prevents double-fire. `applyBulkRating` and `handleCancel`'s bulk-undo branch arm it via `_beginDeferredCompareRefresh(n)`; `moveComparePair` (single rating) arms the same protocol with inline code that is **not** yet equivalent — it discards both `updateMlModelWithFeatures` returns and hardcodes `2` (🟤 BACKLOG [2026-08-30] item 1; delete this clause when that lands). `n` MUST be the count of worker messages actually posted (`updateMlModelWithFeatures`/`reverseMlModelUpdate` return `false` when ML is off or features are missing), or the counter never reaches 0. `_cancelDeferredCompareRefresh()` drops an open window (`loadFolder` — TWICE: before the folder scan, because a scan longer than the 3 s fallback would let it render the old pair mid-await, and again before the empty/non-empty split, because `showLoadingSpinner()` does not set `isLoading` so a window can be armed during the scan; `moveComparePair`'s <2-files exit) and releases `mediaNavigationInProgress` only if a window was actually open — that flag is also held by ordinary in-flight navigation.
...
- The sort-progress card and the plain-text notification **share one DOM element**: `updateProgressNotification` rebuilds it into text form, destroying the bar and Cancel. Anything reporting during a sort must use `updateSortProgress`, and `handleSortByPrediction`'s `finally` is the single owner of teardown — `trainFromHistoricalRatings` clears on no path. Worker `progress` messages are routed by owner (prediction sort → card; similarity sort → left alone; neither → text).

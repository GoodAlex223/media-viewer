...
│   ├── logger.test.js              # logger module: init/log/warn/error/cleanup/getLogPath  [Vitest]
...
- Feature cache v3 schema (TASK-024 design): on-disk per-entry format is `{vector, size, mtime}` (was bare array in v2); `FEATURE_CACHE_VERSION` 2→3 auto-invalidates v2 caches on first load; in-memory `featureCache` Map continues to store `Float32Array` — v3 structure is serialization-only
- featureMetadata Map (`filePath → {size, mtime}`): separate MediaViewer state Map populated at extraction time and cache load time; `saveFeatureCache()` reads from it to write `{vector, size, mtime}` per entry — decoupled from `this.mediaFiles` (which shrinks as files are rated/moved during extraction)
- Feature cache load: builds Set of current filenames from `this.mediaFiles`; entries absent from Set are not loaded (deleted file pruning); size+mtime mismatch causes entry to be skipped (will be re-extracted); next `saveFeatureCache()` writes only what is in the Map, so stale entries disappear from disk
- Feature cache reload on folder switch: `loadFeatureCache()` moved out of lazy-init guard in `handleSortByPrediction()` — called unconditionally before `startBackgroundFeatureExtraction()` so cache is reloaded from disk after every folder switch even when workers are already running
...
- showBackgroundExtractionProgress(current, total, etaText, paused, cachedCount=0): paused=true renders "Paused — N/T (X%) — K cached"; cachedCount persisted to _extractionCachedCount for paused-state redisplay; active state renders "Extracting features: N/T (X%) — K cached"; _extractionLastCurrent/_extractionLastTotal/_extractionCachedCount cache last known values for redisplay when current/total are null
...
- TASK-025 completed (commits 6019189–e0b104c): Application logging to file — new `logger.js` CommonJS module (`init/log/warn/error/cleanup/getLogPath`); `fs.writeSync` writes `[YYYY-MM-DD HH:MM:SS.mmm] [LEVEL] [source] msg\n` to `app.getPath('logs')/media-viewer.log` (overwritten on startup); `main.js` intercepts `console.log/warn/error` after `logger.init()` (source: `'main'`); new fire-and-forget `ipcMain.on('log-renderer-error')` handler routes renderer errors to `logger.warn/error`; `logger.cleanup()` called in `will-quit` (deletes log on clean exit, crash logs survive); `preload.js` exposes `logError: (data) => ipcRenderer.send('log-renderer-error', data)` (never blocks renderer); `media-viewer.js` adds `logError` call in `showError()` + `window.onerror` and `unhandledrejection` global handlers; `logger.js` added to ESLint block 1; `tests/logger.test.js` unit tests added; 3 BACKLOG items spawned (double-init protection, console interception scope, rejection message clarity)
...
- TASK-025 design approved (commit ad9ac3b): Application logging to file — new `logger.js` CommonJS module; `init(logDir)` opens `app.getPath('logs')/media-viewer.log` with `'w'` flag (overwrites on startup); `log/warn/error(source, msg)` write `[TIMESTAMP] [LEVEL] [source] msg` lines; `cleanup()` closes stream and deletes log on clean exit (will-quit) so crash logs survive naturally; main.js intercepts `console.log/warn/error` after init (captures all 12 existing calls without touching them); new `ipcMain.on('log-renderer-error', ...)` fire-and-forget handler; media-viewer.js adds `logError` call in `showError()` plus `window.onerror` and `unhandledrejection` global handlers; preload.js adds `logError: ipcRenderer.send(...)` channel (send not invoke — never blocks renderer); logger.js added to ESLint block 1; new `tests/logger.test.js` unit tests; worker logging, log rotation, renderer console interception out of scope
- TASK-024 (commits 59efdd5–ba4da8e): Per-folder feature cache — cache schema v3 (per-entry `{vector, size, mtime}`, FEATURE_CACHE_VERSION 2→3); `featureMetadata` Map populated at all 6 `featureCache.set()` sites; `loadFeatureCache()` moved out of lazy-init guard (core bug fix: workers survive folder switch, featureCache never reloaded on 2nd+ folder); stale-entry pruning: entries absent from current mediaFiles or with mismatched size/mtime skipped on load; progress indicators show cache hit count — "All N features loaded from cache" early-return notification, `_extractionCachedCount` state for redisplay on paused/resume, "Extracting features: X/Y (Z%) — N cached" during extraction, "Feature extraction complete — N files (M cached, K extracted) in Xs" completion
- TASK-023 completed (commits 9f777c5 + 3f5ea37): Fix video pause/play icon synchronization — root cause: `lucide.createIcons({nodes: [el]})` used a non-existent `nodes` param; Lucide silently ignored it and re-scanned the entire document on every call, replacing all `[data-lucide]` SVGs and invalidating cached `playIcon`/`pauseIcon` refs; fix: changed 3 call sites from `{nodes: [...]}` to `{root: element}` to scope icon creation to the target subtree (media-viewer.js lines ~719 modal, ~2102 zoom popover, ~2651 compare pane); compare mode required splitting one array call into two separate `createIcons({ root })` calls (one per pane wrapper); 2 BACKLOG items added: pin Lucide CDN version, add play/pause icon toggle regression test
- PR #22 code review (commit b3329c0): 5 parallel agents + confidence scoring produced 9 items; 1 above 80/100 threshold fixed in 962414e (saveFeatureCache fallback {size:0, mtime:0}); remaining below threshold added to BACKLOG: update CLAUDE.md Cache Management docs to include `featureMetadata` in `removeFileFromList()` (75/100); update `removeFileFromList` test to assert `featureMetadata` cleanup (75/100); reuse `fileInfo` param in `computeFeatures()` instead of redundant `mediaFiles.find()` (75/100); clear `extractionStartTime` in all-cached early-return path (75/100); fix `_extractionCachedCount` stale after all-cached early-return (75/100); update misleading `FEATURE_CACHE_VERSION` comment (75/100); guard `loadFeatureCache()` against clearing in-memory Maps mid-extraction (75/100); index TASK-024 spec in docs/README.md (75/100); add "Status: Complete" to archived TASK-024 plan (75/100)
- PR #21 code review (commit 70cf595): 5 parallel agents + confidence scoring produced 4 items, all below 80/100 threshold, added to BACKLOG: consolidate duplicate Git Insights entries for TASK-023 in CLAUDE.md (75/100); add `// Use root param...` comment to modal and zoom-popover call sites — only compare-pane site has it (60/100); add Plan: field to TASK-023 DONE.md entry for process consistency (50/100); clarify DONE.md "3 calls" wording — third call was split into 2 making 4 total (50/100)

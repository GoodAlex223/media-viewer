...
**Next planned**: Group A (CLIP extraction silent failure) complete. Group B — AI Prediction Display Bugs: spec at `docs/superpowers/specs/2026-05-14-ai-prediction-display-bugs-design.md`, implementation plan at `docs/superpowers/plans/2026-05-14-ai-prediction-display-bugs.md`, branch `fix/ai-prediction-display-bugs`; `restoreFeatureCachesFromHistory(entry)` helper implemented (69f861b, 2026-05-14: near `removeFileFromList`, 5 unit tests, 180→185); remaining: (2) patch `sortComplete` in `handleMlWorkerMessage` to write `message.scores` into `predictionScores` before reordering; (3) capture `mlFeatures` in `moveToSpecialFolder` history entry; (4) call helper in all 4 `handleCancel` branches. Group C — PR #33 Defensive Follow-ups (clear `clipUnloadTimer` in toggle-off handler, try/catch around `deleteSortCache('clip')`, per-file abort check in `insertNewFilesInSortedOrder`). See `docs/planning/WEEKLY.md` for current schedule.

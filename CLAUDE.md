...
- CLIP toggle-off handler missing `clearTimeout(this.clipUnloadTimer)` (design spec written, Group C): the `if (!clipToggle.checked)` block runs cleanup but does NOT cancel a pending 30s unload timer; race: extraction completes → timer set → user disables CLIP → stale timer remains → user re-enables CLIP + `initClipModel()` begins → stale timer fires `unloadClipModel` IPC mid-load; mitigated today by `main.js` returning `{success:false, reason:'loading'}` (e7d84d0); fix: `if (this.clipUnloadTimer !== null) { clearTimeout(this.clipUnloadTimer); this.clipUnloadTimer = null; }` before the sort-revert block in the toggle-off handler — matches `startBackgroundFeatureExtraction` timer-clear pattern (media-viewer.js:7009-7011)
- `insertNewFilesInSortedOrder` has no abort check (design spec written, Group C): both hash and CLIP branches have no `sortAbortController.signal.aborted` check; cancel during cache-hit insertion is silently ignored; fix: add `if (this.sortAbortController?.signal.aborted) throw new Error('Sort aborted');` at top of the outer `for` loop in both branches (media-viewer.js:5125 CLIP, 5174 hash) — outer-only, not inner hot path; optional chaining safe because `sortAbortController` is null between sorts; error message matches existing pattern at L4223/4254; event-loop yielding (`await new Promise(r => setTimeout(r, 0))`) deferred to BACKLOG pending observed UI freeze
- Integration test gap — unit-test-the-leaf misses call-graph bugs (design spec written, Group D): PR #33's primary fix slipped through existing unit tests because they called `insertNewFilesInSortedOrder` directly with an explicit `'clip'` arg, bypassing the broken `applyCachedSortOrder → cachedData.algorithm` plumbing; lesson: leaf-level unit tests don't catch wiring bugs; fix: `tests/integration/cached-sort-path.test.js` — 3 cases exercising real `applyCachedSortOrder` → real `insertNewFilesInSortedOrder` (CLIP cache-hit, VPTree cache-hit, old cache without algorithm field); uses duplicated `extractAsyncMethod` helper per project convention; orthogonal/parallel unit vectors for deterministic cosine-distance ordering

...
- Shortcut localStorage versioning: `saveShortcut()` persists the FULL object, so a **changed** default binding won't reach existing users — that needs a versioned migration in `loadShortcuts()` (`(custom.version||1) < CURRENT` → delete stale keys, bump, re-persist); guard `typeof localStorage.setItem === 'function'` for read-only test mocks. The migration already exists at **v2** (v1→v2 remapped `next` KeyD→KeyS). Bump only for a changed default — an **additive** key needs none, since a stored object that predates it falls through to the default via `Object.assign({}, DEFAULT_SHORTCUTS[mode], custom[mode])` (how compare's `leftSpecial`/`rightSpecial` shipped).

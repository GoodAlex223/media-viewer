...
**In progress:**
- **CLIP Similarity Sorting** (Group D, 5 SP, branch `feature/clip-similarity-sorting`): Worker implementation complete (9c7fefe). `calculateCosineDistance(vec1, vec2)` and `sortMediaBySimilarityClip(mediaFiles, clipVectors, currentIndex)` implemented in `sorting-worker.js`; 9 unit tests added to `tests/sorting-worker.test.js`; `calculateCosineDistance` exported via conditional CJS block. Renderer-side wiring complete (a538b22): `handleSortBySimilarity()` CLIP branch — `enableClipFeatures` guard, `clipCache` vector collection via `Array.from(vec)`, `vectorCount < 2` guard, info notification, progress notification, **pre-worker abort check** (`sortAbortController.signal.aborted`), then `runSortingWorker({ algorithm: 'clip', mediaFiles, clipVectors, currentIndex })`; `sortedCount = vectorCount`; files without vectors appended at end; `'clip': 'CLIP (semantic)'` added to `algorithmNames` map. Sort cache persistence (`saveSortCache/loadSortCache/deleteSortCache('clip')` → `.sort_cache_clip.json`) also wired in same commit. No E2E tests (impractical without real CLIP inference). **Remaining**: design spec approved — task effectively complete pending final review.
...
- CLIP sorting worker data shape: `clipVectors` is `{path: number[]}` (plain arrays, not Float32Array — Float32Array can't serialize over `postMessage` as plain JSON); renderer must call `Array.from(vec)` when reading from `clipCache` before sending to worker
...
- CLIP sort cache key is the string `'clip'` → produces `.sort_cache_clip.json`; existing `saveSortCache/loadSortCache/deleteSortCache` infrastructure handles it with no new code

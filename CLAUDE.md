...
├── tournament.js        # TournamentManager ES module (v2.0 pattern, mirrors fullscreen.js); IPC glue for tournament state persistence + apply; methods: handleStartClick, handlePairResult, handleApply, handleDiscard, validateStateFile, handleResume, getProgressText, getTierBreakdownText
...
**Data Flow**:
1. Main process handles file system operations (read, move, copy), CLIP model inference (`loadClipModel`, `extractClipEmbedding`, `extractClipEmbeddingBatch`, `unloadClipModel` IPC handlers), and tournament state persistence (`readTournamentState`, `writeTournamentState`, `deleteTournamentState`, `applyTournamentResults` IPC handlers — state file `.tournament_state.json` in the media folder)
...
- Tournament engine tests (pure ESM pattern): `tests/swiss-strategy.test.js` (12 unit, 7 describe blocks) and `tests/tournament-engine.test.js` (14 unit, 5 describe blocks) import directly via `import { SwissStrategy, TournamentEngine } from '../tournament-engine.js'` — no `createRequire` needed because `tournament-engine.js` is pure ESM (not a Worker module); `tests/integration/tournament-flow.test.js` (4 integration) uses real `TournamentEngine` + real `SwissStrategy` end-to-end (happy path N=8, odd N=7 byes, mid-session file removal, serialize+resume); `tests/tournament-manager.test.js` (8 unit, 6 describe blocks, direct ESM import) covers TournamentManager: handleStartClick (N<2 guard + showNotification, N>=2 success + engine created), handlePairResult (records + writeTournamentState called twice), handleApply (applyTournamentResults called with folderPath + {file→tier} assignments), handleDiscard (nulls engine + deleteTournamentState), validateStateFile (valid match / invalid removed+added delta), handleResume (reconstruct from serialized state, history.length preserved), getProgressText ("Round X of Y · Game N/M"), getTierBreakdownText (parts.length === R+1, sum === file count); `makeHost(mediaFiles)` helper returns `{ mediaFiles, showNotification: vi.fn(), baseFolderPath: '/test/folder' }`; `beforeEach`/`afterEach` patches `globalThis.window.electronAPI` with `writeTournamentState`, `deleteTournamentState`, `applyTournamentResults` mocks (restore via `origWindow`); 221→229 unit + 4 integration tests (8 TournamentManager tests added on top of the 221 from engine/strategy phases)
...
**Keyboard Shortcuts**:
- `DEFAULT_SHORTCUTS` top-level constant defines default key bindings for `single`, `compare`, and `tournament` modes (QWER+AD layout); tournament mode: `leftWins: 'KeyQ'`, `rightWins: 'KeyE'`, `undo: 'Ctrl+KeyA'`, `leftSpecial: 'Digit1'`, `rightSpecial: 'Digit2'`
...
- Tournament Escape key: `if (this.isTournamentMode) { this.switchMode('single'); return; }` — exits to single mode with state preserved in `this.tournament.engine` (not discarded); re-entering tournament via selector re-shows the pair display if engine is non-null
- `_checkTournamentResume()` is non-blocking — called near end of `loadFolder()` after `baseFolderPath` is set; reads `.tournament_state.json` via `readTournamentState` IPC; if found, calls `validateStateFile` (strict exact-match of file sets); shows `#tournamentResumeModal` on valid state or an invalidation message on mismatch; user choice (Resume / Discard) is async and overlays the already-rendered media

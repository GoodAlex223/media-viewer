...
**ML Compare Mode Race Condition (TASK-020 design)**:
...
- Flow: moveComparePair() sets pendingCompareRefresh=true, pendingCompareUpdates=2, skips showMedia(); updateComplete handler decrements counter; when 0, bypasses debounce and sends immediate scoreAll; scoreComplete handler calls showMedia() then resets flags
- Undo path: single updateComplete expected so pendingCompareUpdates=1
- Fallback timeout (3s): clears flags and calls showMedia() with stale scores to unblock UI
- Score delta UX: snapshot predictionScores before rating (previousScores Map); diff on scoreComplete; notify "ML updated: N files rescored (X↑ Y↓)" via showNotification() (~2s)
...
- TASK-020 design spec added (commit 16110c1): ML sorting pair ordering investigation — root cause analysis of 6 user-reported issues; Issue 2 confirmed as race condition bug: moveComparePair() calls showCompareMedia() before updateMlModelWithFeatures() 100ms debounce re-score completes, next pair renders with stale scores; fix uses pendingCompareRefresh flag + pendingCompareUpdates counter (2 for rating, 1 for undo) to bypass debounce and defer showMedia() until scoreComplete arrives; score delta UX notification "ML updated: N files rescored (X↑ Y↓)"; unit test file tests/ml-pair-selection.test.js planned (7 test cases); Issues 1, 4, 6 working as designed; 5 BACKLOG items added (content-understanding features, auto re-sort, model diagnostics panel, margin-based pairing, score confidence indicator)

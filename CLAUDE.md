...
- TASK-024 in progress (commits de8ccc4–ef18350 + Task 7): Per-folder feature cache — cache schema v3 (per-entry `{vector, size, mtime}`, FEATURE_CACHE_VERSION 2→3); `featureMetadata` Map populated at all 6 `featureCache.set()` sites; `loadFeatureCache()` moved out of lazy-init guard (core bug fix: workers survive folder switch, featureCache never reloaded on 2nd+ folder); Task 7: progress indicators show cache hit count — "All N features loaded from cache" early-return notification, `_extractionCachedCount` state for redisplay on paused/resume, "Extracting features: X/Y (Z%) — N cached" during extraction, "Feature extraction complete — N files (M cached, K extracted) in Xs" completion

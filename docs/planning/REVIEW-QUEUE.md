...
- _Outbound_ — **A reviewer’s *negative* finding must cite the evidence that establishes it.** Surfaced by this run’s own closeout review: of 5 parallel reviewers, one asserted *"DONE.md correctly left untouched"* and *"no internal contradictions found"* — **both false**, and both refutable by a single `git log` (`a1076a7`/`c4a04b3` wrote DONE.md in-branch, pre-PR) and a single file read (run-card L114 vs L178). A confident denial is **worse than a miss**: a miss leaves the question open, a denial closes it. The findings that survived did so because three reviewers converged independently and the git history was checked first-hand — while the *least*-trusted report on arrival (the cross-repo claim) proved exactly right once the sibling repo was read directly. Tone did not predict accuracy; convergence and primary evidence did. Target is global `POLICIES/code-review.md`, so it is an outbound propagation rather than a 🟤 entry — **deliberately not double-filed**. Held to next run to keep the one-propagation-per-run cadence (this run already spent it on the rating axis, whose sibling this is: that one says *don’t let a filter drop a confirmed finding*, this one says *don’t let an unevidenced denial stand in for a check*).

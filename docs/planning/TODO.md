...
**Last Updated**: 2026-09-02 <!-- Group G6 Weekly Reviews (2026-09-02 run): one § Spawned Tasks row added — propagate "a reviewer's negative finding must cite its evidence" to global `POLICIES/code-review.md` (REVIEW-QUEUE §4 outbound, verdict 2026-09-02; two-trees edit, absence grep-verified at 0 hits in both trees). No Planned/In-Progress item corresponds to G6 — it is ⚪ overhead scheduled straight from WEEKLY.md — so nothing transitions out of here. ⚠️ The row above it, the 2026-08-27 `realness` propagation, is still UNAPPLIED (0 hits at its target); both target the same file and should be applied together. -->
...
- [x] 🟠 **Don't re-train likes/dislikes model when only the source folder changed** — ✅ **Done 2026-09-10** (Group G1 ML training pipeline; branch `g1-ml-training-pipeline`, 10 tasks + per-task review rounds, merged `bfcc881` — [PR #68](https://github.com/GoodAlex223/media-viewer/pull/68), opened on user direction at handover). **The root cause recorded below was wrong, and the correction changes the fix**: `resetMlModel()` never fires on a source-folder change — its call sites are the four like/dislike folder controls (browse/clear × like/dislike) and the CLIP toggle (plus, as of this branch, the Settings "Rebuild model" handler); `loadFolder()` does not call it and never did. The real defect was that the trained model was cached **per source folder** (`.ml_model.json` inside `baseFolderPath`) while trained from **globally-configured** like/dislike folders — so a source folder that had never been AI-sorted found no cache and re-extracted/retrained a byte-identical training set, and (the inverse defect, found during the design pass) an already-sorted folder never retrained again no matter how the like/dislike folders changed. Fixed by keying the model on a training-set fingerprint (like/dislike folder contents + bulk-rated files + `ML_MODEL_VERSION`/`FEATURE_CACHE_VERSION`/`FEATURE_VERSION`/`TRAINING_CONFIG_VERSION`) in a new `MlTrainingManager` module (`ml-training.js`), with per-training-folder vector caches and a fingerprint-keyed model cache in app data (`userData/ml-model-cache.json`, LRU 5) — a source-only switch now hits the cache for free, and the online per-rating update protocol was dropped in favor of rebuilding from cached vectors on each AI-sort request. See spec [2026-09-10-ml-training-pipeline-design.md](../superpowers/specs/2026-09-10-ml-training-pipeline-design.md), plan [archived](../archive/plans/2026-09-10_ml-training-pipeline.md), and [DONE.md](DONE.md) 2026-09-10. _Original:_ _Promoted 2026-08-28 (re-confirmed by manual-testing intake; companion refinements — per-folder change granularity + `feature_cache.json` vector reuse for `trainHistorical` — filed to BACKLOG 🔵 [2026-08-28] and should be weighed in this item's design pass)._ Investigation: why does "Training model from historical ratings…" (reloading the like/dislike folders and retraining) run again after switching only the source folder, when the like/dislike folders are unchanged? `resetMlModel()` on folder change clears `mlStats.isReady`, so the next AI feature re-runs `trainFromHistoricalRatings()`, which reloads both custom folders from disk and re-extracts 576-dim features for every file in them. Consider caching/keying the trained model on the like/dislike folder identity so a source-only switch skips the retrain (русский оригинал: Вопрос: почему начинается обработка лайков/дизлайков для новой папки, если папки лайков и дизлайков не менялись, а поменялась лишь исходная папка?); affected: `media-viewer.js:~7480-7582` (`trainFromHistoricalRatings` reloads like/dislike folders), `media-viewer.js:~7923` (`!mlStats.isReady` retrain gate), `media-viewer.js:~6913` (`resetMlModel`) [possible-dup-of: Skip like/dislike folder re-processing via change detection (re-report) — BACKLOG 🔵 [2026-08-28]]
...
- [x] **Propagate the code-review rating axis — score by _realness_, not severity — to global `~/.claude`** — _Origin_: REVIEW-QUEUE.md §4 outbound, verdict `propagate` (2026-08-27); learnings from PRs #59/#64/#65/#66. _Target_: the **synced product**, so this is a two-trees edit — `claude-code-universal-config/home-claude/POLICIES/code-review.md` **and** live `~/.claude/POLICIES/code-review.md`. _What to add_, two clauses: (1) a confirmed defect must not be dropped because it is hard to hit, cheap to fix, or comment-only — frequency and recoverability are **not** realness; a numeric severity filter is a proxy that structurally caps some real findings below its own threshold; and (2) a shared-state/lifecycle race that **pre-exists** a PR becomes that PR's finding when the PR adds a **new consumer** whose correctness depends on the broken invariant. _Why it earns a global slot_: it fired on **four consecutive PRs** here, and the one time it was deferred it cost a **23,559-entry / 126 MB** feature cache of real user data (PR #64). _Verified absent at the target_: `realness` / `severity axis` / `confirmed bug` return **0 hits** in both trees' `POLICIES/code-review.md`. The one adjacent hit (global L266, "raised below its own reporting threshold and passed on informally rather than posting") governs **bookkeeping** — every remark gets a bullet — and is complementary: this rule is upstream of it. _Source of truth here_: memory files `feedback_defer_lifecycle_findings.md`, `feedback_review_threshold_comment_findings.md` (per-project, reach no other repo by any route). Status user-maintained — not verifiable from this tree.
  ✅ **Applied 2026-09-24 (G5 Weekly Reviews), by user ruling** — live `~/.claude/POLICIES/code-review.md`: first bullet of a new *Deciding What to Report* subsection under *Conducting Review*, plus a pointer (item 8) in *Claude's Role in Reviews*. Post-edit `grep -c realness`: live **3** (was 0); `claude-code-universal-config/home-claude` still **0** — expected under live-first, since that tree receives changes only through its own one-way sync. Outcome row in [REVIEW-QUEUE.md](REVIEW-QUEUE.md) § 4.
- [x] **Add a Closeout-artifacts block and a live-surface preflight step to the global plan template** — _Origin_: Group G3 Task 5 (2026-09-02); BACKLOG 🟤 `[2026-08-27] From: G5 closeout` + `From: G4 closeout` ×2. The **in-tree half shipped** on this branch — [plans/README.md](plans/README.md) now carries both, and [README.md](README.md) carries the derived-count convention. _Target_: `.claude/TEMPLATES/plan.md`, which is **global and gitignored**, so it cannot be edited from this tree; per the live-first rule the live `~/.claude/` tree is edited first and `claude-code-universal-config` receives it via the one-way sync. _What to add_: (1) the **Closeout artifacts** table — every artifact the global `CLAUDE.md` § Task Completion step names (BACKLOG · TODO · DONE · WEEKLY · `docs/README.md` · `docs/archive/plans/`), each marked done or **N/A with a reason**, derived from the rule rather than hand-written; and (2) a **live-surface preflight** step — list every live surface asserting the claims the task corrects (`CLAUDE.md`, `PROJECT.md`, READMEs, prompts, templates), then diff that list against the plan's Files Affected. _Why global_: both defects are template-shaped, not repo-shaped — a hand-written Outputs list is executed instead of the rule it implements, and its omissions read as completeness. Copy the wording from [plans/README.md](plans/README.md) § Closeout artifacts and § Creating a Plan step 4. Status user-maintained — not verifiable from this tree.
  ✅ **Applied 2026-09-24 (G5 Weekly Reviews), by user ruling** — live `~/.claude/TEMPLATES/plan.md`, template version 1.0 → 1.1: a **Live-surface preflight** paragraph under *Files Affected*, and a new **§ 9 Closeout Artifacts** table whose rows are derived from the global Task Completion rule — including the step-3 clause propagated in the same run (the last row of this section). Post-edit `grep -c`: `Closeout Artifacts` live 1, `Live-surface preflight` live 1 (both 0 before); sibling tree 0 → sync pending. Outcome row in [REVIEW-QUEUE.md](REVIEW-QUEUE.md) § 4.
- [x] **Propagate "a reviewer's _negative_ finding must cite the evidence that establishes it" to global `~/.claude`** — _Origin_: REVIEW-QUEUE.md §4 outbound, verdict `propagate` (2026-09-02); surfaced by the 2026-08-27 closeout review and fired again inside this window (the G5 closeout review asserted a false premise that three further comments restated; fixed by deleting the line, `e2b5724`). _The rule_: a reviewer asserting that something is **fine** must cite what establishes it. A confident denial is worse than a miss — a miss leaves the question open, a denial closes it. On 2026-08-27 one of five reviewers asserted "DONE.md correctly left untouched" and "no internal contradictions found"; **both false**, both refutable by a single `git log` and a single file read. What predicted accuracy was independent convergence plus primary evidence, never tone. _Target_: the **synced product**, so this is a two-trees edit — `claude-code-universal-config/home-claude/POLICIES/code-review.md` **and** live `~/.claude/POLICIES/code-review.md` (live-first per the one-way sync rule). _Absence verified, not assumed_: `negative finding` / `cite its evidence` / `unevidenced` / `confident denial` / `denial` return **0 hits** in both files (382 lines each, both confirmed present, so the zero is a real absence and not a missing-file artefact). _Not verifiable from this tree; status is user-maintained._
  ✅ **Applied 2026-09-24 (G5 Weekly Reviews), by user ruling — in one edit with the realness row**, as the burn-down warning that stood here proposed (the two are companions: one says _don't let a filter drop a confirmed finding_, the other _don't let an unevidenced denial stand in for a check_). Live `~/.claude/POLICIES/code-review.md`: second bullet of *Deciding What to Report*. Post-edit `grep -c "negative finding must cite"`: live 1 (was 0); sibling tree 0 → sync pending. That warning — the outbound channel at **2 filed, 0 applied**, the zero-burn-down pattern § 5 was built to fix for `adopt` — is closed: REVIEW-QUEUE § 4 now keeps an outbound outcome log, and every propagation filed so far is applied live.
- [x] **Propagate "a closeout checks off every BACKLOG/TODO entry the task closed, in the closeout commit" to global `~/.claude`** — _Origin_: REVIEW-QUEUE.md §4 outbound, verdict `propagate` (2026-09-24) — the planning-time lesson the Sep 7–11 plan recorded twice: G2's closeout (`1d8868c`) flipped none of the four PR #65 BACKLOG entries it shipped (four, as G4 corrected the planning note's "five" — the fifth, the CLAUDE.md line-191 split, was never performed), and G1's three WEEKLY task boxes were flipped by G4 rather than by G1's own closeout. _Target_: `~/.claude/rules/planning-closeout.md` § Task Completion step 3, which checked off only the WEEKLY group, **and** `~/.claude/WORKFLOW.md` § Phase 5 — the second live description of the same step (its step-3 paragraph and the 5.2 checklist), included so the two do not diverge. _Absence verified with a positive control_ before writing: a step-3 vocabulary pattern set returned 0 in `rules/planning-closeout.md`, `WORKFLOW.md`, `CLAUDE.md` and `TEMPLATES/plan.md` in both trees, while the control phrase `check off the shipped group` returned 1 in `planning-closeout.md` in each. ✅ **Applied 2026-09-24 in the same run, by user ruling** — post-edit `grep -c "entry the task closed"`: planning-closeout live 1, WORKFLOW live 2; sibling 0 → sync pending (live-first). Also carried as a row of the new template § 9 table (the Closeout-artifacts row above). Outcome row in [REVIEW-QUEUE.md](REVIEW-QUEUE.md) § 4.

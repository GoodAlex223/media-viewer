...
- [x] **Propagate "a closeout checks off every BACKLOG/TODO entry the task closed, in the closeout commit" to global `~/.claude`** — _Origin_: REVIEW-QUEUE.md §4 outbound, verdict `propagate` (2026-09-24) — the planning-time lesson the Sep 7–11 plan recorded twice: G2's closeout (`1d8868c`) flipped none of the five PR #65 BACKLOG entries it shipped, and G1's three WEEKLY task boxes were flipped by G4 rather than by G1's own closeout. _Target_: `~/.claude/rules/planning-closeout.md` § Task Completion step 3, which checked off only the WEEKLY group, **and** `~/.claude/WORKFLOW.md` § Phase 5 — the second live description of the same step (its step-3 paragraph and the 5.2 checklist), included so the two do not diverge. _Absence verified with a positive control_ before writing: a step-3 vocabulary pattern set returned 0 in `rules/planning-closeout.md`, `WORKFLOW.md`, `CLAUDE.md` and `TEMPLATES/plan.md` in both trees, while the control phrase `check off the shipped group` returned 1 in `planning-closeout.md` in each. ✅ **Applied 2026-09-24 in the same run, by user ruling** — post-edit `grep -c "entry the task closed"`: planning-closeout live 1, WORKFLOW live 2; sibling 0 → sync pending (live-first). Also carried as a row of the new template § 9 table (the Closeout-artifacts row above). Outcome row in [REVIEW-QUEUE.md](REVIEW-QUEUE.md) § 4.

...
<!-- [2026-06-19] Decomposed (brainstorm → spec → plan) into 3 staged PRs under a strict no-quality-change constraint (user: "quality must not change at all"):
     ...
     NOTE: PR1 made the sort non-freezing/transparent/cancelable but, being quality-locked, did NOT reduce the core O(n·K) neighbor-graph build time — PR2/PR3 remove the hashing + cache-load waits but NOT the O(n·K) neighbor-graph-build floor (moved only by #7 parallel build or a relaxed quality-lock K-cap); the AI-prediction sort has no graph build and is fully addressed by PR3 + PR1. Item stays 🔴 OPEN until all 3 land (PR1 smoke passed 2026-06-19; PR1 MERGED 2026-06-20 via PR #54). -->

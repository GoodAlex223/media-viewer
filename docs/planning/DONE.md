...
- **§ 5 context-cost audit → `keep`, and acted on.** A media_viewer session loaded far more than the nine plugins the entry named: **eight plugins synced from the user's claude.ai account** (`sales`, `small-business`, `legal`, …) that Claude Code had never once invoked. Measured with Claude Code's own estimator (`claude plugin details`), not the UI. Disabled user-wide on the user's ruling, along with `pr-review-toolkit` (project scope), `hookify` and the `context7` plugin (this repo only), `dead-rules-audit` (**uninstalled**) and four dangling settings entries. Plugin always-on context per session fell from about 24k tokens to about 1.5k, by the estimator — a floor, since it does not count hook-injected context.
...
**Branch**: `g1-ml-training-pipeline`, cut from `main` at `357800d`. **Merged to `main` 2026-09-12 as `bfcc881`** (`--merge`, PR #68; branch deleted local + remote). Every task shipped with its own per-task review round (several with fix rounds); the whole-branch merge was a separate, later decision, taken after four PR review rounds closed with no issues found. The task brief said **no PR** and review ran locally, task-by-task, instead; at handover the user chose "push and create a Pull Request", so the branch is on [PR #68](https://github.com/GoodAlex223/media-viewer/pull/68).
...
**Tests**: unit 613 → **761** (every commit green); E2E 56 → **61**, run clean on the final revision. The unit tests added after the ten planned tasks come from the whole-branch review fix wave and the PR #68 review round, both recorded below. This entry is the **only** place the branch's test delta is stated — WEEKLY and TODO point here rather than restating it, per [README.md](README.md) § Document Conventions.
...
which, as the final whole-branch review found, needed a second enforcement point in the host's own
`computeFeatures` before it was actually true; see below),
and a fingerprint-keyed model cache in app data (`userData/ml-model-cache.json`, 5 slots evicted by
write-recency, atomic `.tmp`+rename). `ensureTrainedModel()` resolves in three tiers — live session (free) → persisted
...
- **`media-viewer.js`** — **net smaller** across the branch, from 9,642 at the fork point (exact end count deliberately not restated: it moved with every review round and would go stale again on the next commit — `wc -l media-viewer.js` at the merge commit is the live figure) (Global Constraints required the new module to remove more than it added; Task 7 alone: 156 insertions / 983 deletions): `trainFromHistoricalRatings`, `trainFromHistoricalRatingsAndWait`, `collectBulkRatedTrainingExamples`, `updateMlModelWithFeatures`, `reverseMlModelUpdate`, `_beginDeferredCompareRefresh`/`_cancelDeferredCompareRefresh`, `loadMlModel`/`saveMlModel`/`deleteMlModelCache` all deleted; `handleSortByPrediction` now calls `mlTraining.ensureTrainedModel()`; Settings gains a "Rebuild model" control (`handleRebuildModelClick`).
...
- ✅ **This entry was written before the branch was merged, by design, not by oversight** — and the merge then falsified **26** live claims across four documents, exactly as BACKLOG `[2026-08-31]` item 2 predicts ("closeout is written before the merge, and the merge falsifies it"). Holding `◐ Branch complete, unmerged` until the merge is what made that sweep a mechanical grep for one phrase rather than a hunt. All 26 were flipped in the first commit after merge `bfcc881`: the WEEKLY status cell and daily row, this heading and Branch line, 20 BACKLOG parentheticals and one in TODO. ⭐ The measurement is the argument for the convention: **a pre-merge closeout is correct when written and wrong the moment it lands**, so the only safe status before a merge is one that says so.

**PR #68 review round** (2026-09-12, after the branch was pushed on user direction): 5 findings + 2 below-threshold in round 1, all resolved in one wave; rounds 2 and 3 verified every one against the pushed revision (mutation-testing each new guard), accepted both corrections to round-1 text, and each left exactly one item, both fixed below.

- **1 real correctness defect.** `_writeCachedModel` is a read-modify-write across two IPC round trips and `invalidateModelCache` writes the same file, with **no mutex** — the feature-cache path in this same branch is serialized for exactly this reason, and the model cache had no equivalent. A "Rebuild model" click landing inside that window was silently undone: the write re-persisted the entry list it had snapshotted before the clear, putting back the very entries the user was told had been discarded. Fixed with a dedicated `_acquireModelCacheLock` (deliberately *not* the host's feature-cache mutex, which a streaming read can hold for tens of seconds — parking the model-cache lookup behind it would forfeit the instant cache hit this branch exists to produce). The same click was also ungated during an in-flight sort, where `resetMlModel()`'s `{type:'reset'}` zeroes the weights the sort is about to score with; now refused with a warning.
- **4 documentation defects**, three of them stale derived values — a line-number pointer that had rotted by 60 lines, and two counts (unit tests, renderer line count) that this branch had *already* corrected once each. Both counts were re-broken by the review wave that corrected them, which is the whole argument for [README.md](README.md) § Document Conventions: they are now deleted or pointed at a single authoritative statement rather than refreshed again.
- ⭐ **A reviewer's finding can be right while the contract it cites is wrong.** Finding 5 correctly flagged that `o.loadModelState`'s JSDoc omits `modelWasReset`, which its own consumer depends on — but described the promise as resolving the worker's full six-field `initComplete` message. It does not: the renderer's handler narrows it to `{stats, modelWasReset}` before resolving. Documenting the cited contract would have shipped four fields that never arrive. The fix documents the real two, plus the narrowing.
- **Round-2 residual — an entry-only guard is half a guard.** The `isPredictionSorting` check added above sat at function entry, while `resetMlModel()` ran unconditionally after an `await`. A sort started *inside* that IPC round trip hit the identical worker-zeroing failure the gate was added to prevent, with the two clicks in the other order. Reachability is genuinely narrow (the sort control is click-only, `media-viewer.js:1917`, and the window is one IPC round trip) — fixed anyway, because the corruption is the same class as the finding it followed and the remedy is a re-check: the clear still happens, the running sort keeps its model, and the notification says so. ⭐ The generalisable half: **a guard placed before an `await` does not hold after it**, and adding one is the moment to ask what else can change while the awaited call is in flight.
- **Round-3 item — fixing the write side left the read side ordered by luck.** Round 1 made `_readCachedModel` deliberately lock-free, arguing that the writer renames a fully-written temp file so a reader sees "the old store or the new one, never a torn one." That is sound about tearing and silent about **ordering**: immediately after a user-requested clear, seeing the old store *is* the defect — a sort racing the clear is served the entry the user asked to discard and then pins it as `sessionFingerprint` for the rest of the session, which is the same corruption finding 1 was about. The read now takes the same lock; the cost is a microtask when uncontended, since this lock is only ever held across one small read or write of one file. Surfaced while verifying round 3's wording nit, which had assumed the racing sort always retrains — true only once the read is ordered. ⭐ **A rationale can be correct about the hazard it names and silent about the one that matters**: "never torn" answered a question nobody was going to lose data to, while the ordering question it did not raise was the live one.
- **Round-3 wording nit — fixed.** The skipped-reset notification claimed the running sort "keeps the current model; the next sort will rebuild." Both halves are wrong whenever the racing sort is still upstream of `ensureTrainedModel`: it misses both tiers and retrains, so it *is* the rebuild. Which of the two happens depends on where the sort had got to, so the message now asserts neither — "retrains on that sort or the next one."
- **Pushed back on one sub-item**: the shared `.tmp` path in `write-ml-model-cache`. It is the identical pattern used by `writeTournamentState` and the feature-cache writer, both of which are safe purely because the renderer serializes their callers — which is precisely what the mutex above now also gives the model cache. Treating this one writer differently would diverge from the house invariant while fixing nothing the lock does not; filed as 🟤 `[2026-09-12]` to consider for all three writers together.

**Follow-up tasks**: BACKLOG 🟤 `[2026-09-10] From: G1 ML training pipeline — execution review follow-ups` — sourced from the closeout dispatch's explicit mandates, the task brief, a pre-existing observation, the closeout's own judgment pass over the execution ledger, and (added later) the whole-branch review fix wave below; one entry in it is **resolved** by that wave, and another **annotated** with a reachability correction. Additional pre-existing BACKLOG and TODO entries were also ruled on across the closeout commit and its fix rounds — see those entries for which, not a restated total here.

**Final whole-branch review fix wave** (after closeout, before merge — the last work on the branch).
A review over the full `357800d..HEAD` range found one Critical, four Importants and a set of
minors; all were re-verified against the code before being fixed, and each of the three headline
findings was reproduced by a RED test first.

- **Critical — `resetMlModel()` left the manager's session tier claiming a model the worker no longer had.** `{type:'reset'}` zeroes the worker's weights and both class counts, but nothing touched `mlTraining.sessionFingerprint`, and `ensureTrainedModel` resolves its `session` tier by fingerprint equality alone. Since every `resetMlModel()` call site returns the descriptor to the *same* fingerprint (a CLIP toggle off-then-on, re-picking the same like/dislike folder), the next sort took a session hit against an **empty** model: stale ready-looking stats to "🧠 Model reused — N👍 M👎", readiness gate passes, then `scores: null, reason: 'Need more samples (0 likes, 0 dislikes)'`, repeating for the rest of the session. A **regression** — before this branch, `mlStats = null` alone re-armed the `!this.mlStats?.isReady` retrain gate and the reset was self-healing.
- **Important — the § 4.1 isolation rule was violated in production, and the test could only see the mock.** The host's `computeFeatures` wrote `this.featureCache.set(filePath, features)` for every like/dislike-folder file the manager extracted. Two traced costs: it inflates `_saveFeatureCacheLocked`'s shrink-guard denominator (the guard that caught a real 23,559 to 32-entry overwrite), and its `has()` short-circuit hands back a **pre-modification** vector for a file the manager just rejected as stale, which is then persisted under the new stat and trained into a model cached under the new fingerprint. Fixed with a `useHostCache` option passed `false` from the manager's injected callback. ⭐ **The isolation test asserted the property against a `vi.fn()` `computeFeatures`, so it was structurally incapable of failing** — the violation lived entirely in the collaborator the mock replaced. Re-pointed at the real method via this repo's `extractAsyncMethod` harness; it now fails with all six training-folder paths in the host map, and a mutation that drops only the flag from the injection line still fails it.
- **Important — a "worker created a fresh empty model" reply was accepted as a successful cache load.** `_runWorkerInit` dropped `modelWasReset`, and `_safeLoadModelState`'s only check was `!loaded?.stats` — an empty model's stats object is truthy — so `source: 'model-cache'` was returned for a model that was never restored (spec § 7.2 requires a malformed entry be treated as a miss). Forwarding `modelWasReset` is **necessary but not sufficient**: measured against `ml-worker.js`, when `savedModel` is truthy but `savedModel.weights` is absent, `initializeModel` never enters its compatibility branch and `modelWasReset` stays **false** while a fresh empty model is still built. Both were implemented — the flag, plus agreement with the entry's persisted `stats` (written since Task 4 and read by nothing until now). Deliberately not gated on `stats.isReady`, which would make a legitimate 2-like/5-dislike model rebuild forever.
- **Important — `initializeMlWorker()` had no backstop timeout**, unlike the sibling `initializeFeaturePool` (given one for exactly this reason). Its prior justification enumerated settle *paths* rather than excluding non-settling ones; a worker that constructs, parses and then never replies without raising `error` hung `await Promise.all([...])`, latching `isPredictionSorting` and pinning the progress card for the session. Symmetric 5 s backstop added.
- **Important (reporting half only) — the per-folder vector load had no progress phase.** The manager retains no vectors across calls, so every rebuild re-streams both training folders' caches in full, and in the rate-then-sort workflow every sort *is* a rebuild. The card now reports `'Loading cached likes'` / `'Loading cached dislikes'`, determinate against the on-disk count. Cross-call retention was **not** implemented — filed to BACKLOG pending measurement.
- **The one-sided-training-set guard, fixed but NOT the way its BACKLOG entry proposed.** ⭐ That entry's suggested `&&` to `||` is **wrong**: the guard runs before `_resolveBulkRatedFiles`, so an empty dislike *folder* whose 'bad' class is supplied by bulk-rated corrections in the source folder is a legitimate, working configuration `||` would silently disable — verified by applying `||` and watching two tests fail. The shipped guard is per **class**, over all of that class's sources, and is suppressed by `scanFailed` so § 7.2's "train on whatever is available" row stays intact. The BACKLOG entry is now resolved **and** its proposed fix corrected in place, so nobody re-applies it.
- **Minors**: training-set scan order normalised in `ml-training.js` (`trainBatch` shuffles an index array *derived* from scan order, so the fingerprint seed alone did not make weights reproducible — a real hole in § 5.4's "a cache hit is verifiable" claim). ⚠️ **A matching sort in `main.js` was added and then reverted in the re-review**: measured on this NTFS volume, `fs.readdir` already returns **case-insensitive** alphabetical (`apple, banana, img_1, IMG_2, Photo1, Zebra`), while a code-unit comparator yields `IMG_2, Photo1, Zebra, apple, banana, img_1` — so it would have reordered the app's user-visible **browse** order for any mixed-case library (`IMG_1234.JPG` beside `img_1234.jpg`), for a reason that has nothing to do with browsing. The E2E fixtures are all lowercase, which is why 61/61 said nothing about it. `ml-training.js` re-sorts unconditionally and is the unit-tested guarantee, so nothing depends on `main.js`'s order. The duplicate `trainComplete` training toast removed (D6 made the indicator the once-per-sort report, and `showNotification` evicts the oldest of five — the sort card **is** a notification element); the never-read `_mlWorkerVersions.featureDim` dropped; and this entry's own commit range re-notated as "through ... inclusive" (`bcd6fa9..e6a37f9` counts 17, not the 18 stated — the count was right, the notation falsifiable in one command).
...
✅ **Status: 5/5 + G4's terminal read-out. MERGED 2026-09-03 via PR #67** (merge `de4bdac`, branch deleted remote + local). The only group this week to take a PR; docs-only throughout, so methodology rule #3 applied and `/code-review`'s agent fan-out was skipped in favour of the docs-only path. **Three review rounds, 8 findings, all resolved** — every one a stale-or-unverifiable factual claim in a live doc, none a defect in the reviews themselves. Two rounds corrected the reviewer's own reasoning rather than only my text (the `CLAUDE.md` size was a **unit** error, not a transcription one; and the odd broken link sits inside the date range the review thought excluded it), and two findings were things the review had not flagged that the fix sweep turned up.

**Summary**: Fourth run of the recurring Weekly Reviews batch, held **2026-09-02 — two days ahead** of its Friday Sep 4 slot, matching G3 and G5. Carries **G4's terminal item** (the `dead-rules-audit` scorecard read-out) on the same branch, which closes G4 at 3/3. Five verdict rows across §§1–4 plus **two § 5 trial read-outs** — the first run in which § 5 had anything to close, and the first in which the adopt queue turned over rather than only growing.

**The headline is that both trials failed, and failed differently.**

- **`dead-rules-audit` → `drop`** as a compliance tool (kept installed, relevance heat-map only). WEEKLY's verdict rule anticipated a vehicle that produced no data; the vehicle produced **13 sessions, 6,499 relevance hits, 2,134 judgements, 1,136 "violations"**. The scoring is what fails. Measured with the plugin's own `parseRules` + `judge` against this `CLAUDE.md`: `ruleKeywords` never separates a rule's **prescribed** tokens from its **prohibited** ones, so `judge()` flags a violation whenever any backticked token from the rule appears in added live code. Rule 36's tokens are `loadfolder`, `exittournamentmode`, `tournament`, `engine` — all prescriptions. Two snippets obeying rules 36 and 37 verbatim both returned `violated: true`. End-to-end confirmation: **10 of 43 rules flagged `⚠ promote→hook`**, headed by the two tournament rules **G2 shipped and enforced the same week**. Cost was never the objection (`scoreDiff` 10.2 ms/edit, `parseRules` 3.8 ms). No re-trial — the defect is in the scoring function, not the vehicle.
- **Visual verification → `inconclusive`**, and a re-trial filed. G5 shipped a **progress-card** change — a thing whose whole purpose is to be looked at — with **zero** visual or behavioural evidence, past five reviewers and an adversarial pass. Verified rather than assumed: `git log --name-only` over the G5 range shows no image of any kind, and DONE.md greps clean for `screenshot` / `visual` / `ran the app` / `npm start`. Its verification was unit 597 → 613 + E2E 56/56, which § 5 already said does not discharge the practice.

**Verdict rows** (5, all dated 2026-09-02):

- **§1a Plugins / official store** → `github` and with it the never-before-reviewed **External integrations** category — **`pass`**. Roster re-read first-hand and unchanged in the six-day window; eleven of the twelve members have no counterpart in a solo, local, no-CI Electron project, and `github` loses to `gh`, already in daily use and free where an MCP charges per-turn context.
- **§1b Plugins / wider internet** → `fracalo/electron-playwright-mcp` — **`pass`**. The **first §1b candidate ever to clear this section's standing Electron gap**, and it still passes on two independent grounds: 1★/15-commit/ISC unvetted trust against an app running `sandbox: false` with file move/delete IPC, and the capability is **already in this tree** (`rdp-preload.cjs` + `electron-wrapper.cjs` already solve the Electron-30+ `--remote-debugging-port` problem; `launchApp()` already drives the real app).
- **§2 Claude best-practice** → **audit installed plugins for context cost and disuse** — **`adopt`** → 🟤. Nine plugins now load every session, never measured; the docs now expose `Context cost`, `Last used` and a **Not used recently** group. First subject is `dead-rules-audit`, whose keep-installed call this run made on a timing figure, not a token figure.
- **§3 Non-Claude best-practice** → **loop engineering / evidence-gating** ("DONE is a claim until evidence supports the transition") — **`adopt`**, gate half only → 🟤. Would have been a `pass` as validation, like Fowler and Osmani, except this run **measured the norm failing**.
- **§4 Cross-project propagation** → **`propagate`** "a reviewer's _negative_ finding must cite its evidence" → TODO § Spawned Tasks, target global `POLICIES/code-review.md`. Absence grep-verified (0 hits across both trees, 382 lines each). Inbound: `/wayfinder`, Jenkins and the design videos **re-verified still pending** upstream at `7dbfd15`, parked for the second consecutive run.

**Key changes** (6 files):

- **[REVIEW-QUEUE.md](REVIEW-QUEUE.md)** — 5 verdict rows + 1 inbound row; **2 § 5 trial outcomes** resolved from `pending`; 2 new `pending` rows for this run's adopts; every section's _Next-up_ refreshed (harness observability's gate condition resolved; the §4 consumed item promoted out); a dated § 5 stocktake update.
- **[BACKLOG.md](BACKLOG.md)** — `### [2026-09-02] From: Weekly Reviews (2026-09-02 run) (3 items)`: two adopt-derived, each naming its trial vehicle, plus one non-adopt item the closeout's own link check surfaced (broken relative links in this file's pre-June 2026 entries — the entry deliberately states no count, having had one wrong). The `dead-rules-audit` trial entry flipped to `[x]` with its outcome.
- **[TODO.md](TODO.md)** — one § Spawned Tasks row for the propagation, with its grep evidence and the burn-down warning.
- **[WEEKLY.md](WEEKLY.md)** — G6's five boxes + G4's bullet checked, Friday rows checked, both Summary-Table statuses flipped; G6's task count corrected 4 → 5 (the read-out item was added after the table was written) and the overhead-inclusive total 23 → 24.
- **[docs/README.md](../README.md)** — run-card indexed in Design Specs (now hook-enforced by `check-docs-index.js`, so the 2026-08-27 miss cannot recur silently).
- **Run-card** — decisions D1–D4 and the appended Outcome.

**Key decisions / learnings**:

- ⭐ **A trial can produce abundant data and still be worthless — check that the metric measures what it claims before reading its output.** `dead-rules-audit` was adopted on a *parser* measurement (36/10, later 38/11, now 42/13/12) and nobody validated the *judge*. The read-out rule written to protect the trial guarded the opposite failure (too little data), so a scorecard full of numbers would have passed straight through it. The tell was in the output all along: the rules it nominated for promotion were the ones the repo had just enforced.
- ⭐ **Order of operations was load-bearing, by design and in fact.** D4 put the § 5 read-outs before §§1–3 because the queue cap parks new adopts while >3 trials are outstanding. Three were. Closing two first let §§2/3 file normally; the reverse order would have parked a legitimate adopt behind a cap that was about to lift.
- ⭐ **Both adopts came out of the failures, not the web.** The §2 and §3 picks are traceable to what the read-outs measured — plugin cost, because this run chose to keep a dud installed; evidence-gating, because a practice with no gate was measurably never performed. The six-day window produced no plugin adopt at all, exactly as the run-card predicted, and that was the honest answer rather than a shortfall.
- ⭐ **`propagate` had the same zero-burn-down disease as `adopt`, unnoticed because § 5 covers adopts only.** Verifying the target for this run's propagation also revealed that 2026-08-27's (`realness`) was **never applied** — 0 hits at the target. Outbound is 2 filed, 0 applied. Recorded on the row and surfaced for the next planning conversation rather than fixed unilaterally.
- ⭐ **The negative result was the most useful §1b row ever written.** The Electron gap that passed three prior candidates was finally cleared by one — and the review still passed it, because the capability was already in-tree. That is what established the visual-verification failure was never a tooling problem, which is what makes the §3 adopt a *gate* rather than another tool.
- ⭐ **A verification grep that cannot match returns 0, which reads exactly like a clean sweep.** Round 2 of the PR review found one stale figure the round-1 fix had missed — and the real defect was the check: it grepped the prose spelling `206 to 211` while the surviving occurrence used a Unicode arrow (`206 → 211`), so it matched nothing anywhere and was reported as clean. The general form is `critical-thinking.md` Phase 4: **verify the check before trusting its zero.** Two habits came out of it — sweep for the **values** rather than one prose rendering of them, and give every sweep a **positive control** that asserts it can still find a known instance, so a dead check reports itself. Same session, second time a zero was mistaken for evidence; the first was the `dead-rules-audit` read-out rule, which guarded "zero scored edits" while the real failure was thousands of invalid ones.
- 🔴 **This branch broke an existing written convention three times, and the convention already named both the failure and the cure.** [planning/README.md](README.md) L58–66 says: _"Do not restate a count or range that is derived from a list elsewhere — reference the list open-endedly instead… The fix that held was deleting the value, not refreshing it,"_ with a review-time tell — _"verify whether the same commit extended the source the fix just described."_ All four PR-review findings of this shape are instances — the fourth landing **inside the entry the first fix filed**, where the recorded count (27) was itself wrong (26 broken, 25 of them fixable by the substitution the entry prescribed, one needing a different target entirely): DONE.md's `(2 items)` after `a4d72a4` added a third; the run-card's Outputs list assuming adopts are the only source; the positive control's expected count of "three", where the same commit added two more matching mentions to DONE.md; and the broken-links entry's "27 … every one off by one level", which was 26, of which one is not off by a level at all and would have survived the fix the entry prescribed. The reviewer found the third and fourth by applying that documented tell verbatim. **The fourth is the one that stings**: it is a restated derived count filed 21 lines above `BACKLOG.md:333`, the already-open item _"Sweep the repo's existing restated derived counts — the convention landed, the sweep did not."_ Two things generalize past this PR. **A convention is not a control** — this one was written, correct, specific, and violated three times in the branch that came after it, which is the same "advisory rules are not self-executing" result § 5's visual-verification read-out produced from the opposite direction. And **the cure for a derived count is deletion, not accuracy**: the fix that held here replaced the expected count with an expected _property_ (`> 0`), because a calibrated count fails on a correct tree the moment prose changes — an expected value that goes stale is a dead check, which is what the sweep learning above is about.
- ⚠️ **Second consecutive run in which an aggregator asserted an official plugin that does not exist** ("Frontend Design" again, plus Semgrep and Chrome DevTools). Roundups are leads to check against the primary doc, never sources for a verdict — now stated as a standing note in §1 Next-up rather than a per-run observation.

**Follow-up tasks**: BACKLOG 🟤 `[2026-09-02] From: Weekly Reviews (2026-09-02 run)` (3 entries — 2 adopt-trial, 1 incidental broken-links defect); TODO § Spawned Tasks (1 propagation, plus the 2026-08-27 one still unapplied). Open for the next planning conversation: whether to extend § 5's trial-log discipline to `propagate` verdicts, and whether a "runner-up" that is really a non-candidate is worth parking (three consecutive runs where §2's fresh check beat the parked list).

### 2026-09-02 — Group G5: AI-sort training-phase progress polish 🔵 (Cleanup Week #3) — **2/2, MERGED `0d3fed5`**

**Plan**: none — brainstormed as a **bounded** change (both items are small edits to code paths that already existed); no plan document, so nothing to archive.
**Spec**: none.
**Merge**: `0d3fed5` (`--no-ff` into `main`, **no PR** — reviewed locally by 5 parallel reviewers plus an adversarial pass; branch deleted local + remote)
**Commits**: `595ec19` (both items + 2 fixes beyond the brief) → `e2b5724` (review round)

**Summary**: The AI sort's cancelable progress card now survives every phase, and the historical-ratings loop no longer trains on silently-corrupt feature vectors. The Cleanup Week's sanctioned 🔵 exception; closes both `### [2026-08-28]` manual-testing entries (batches 2 + 4).

**Key changes**
- `trainFromHistoricalRatings`: all four progress calls → `updateSortProgress` (the card form, with Cancel) instead of `updateProgressNotification`, which rebuilds the shared element as plain text and destroyed Cancel for the longest phase. Per-file counts, not every 10th.
- `collectBulkRatedTrainingExamples`: was reporting **nothing** — a third silent window on the same phase. Now reports, with the count advancing *before* the `continue` so the bar still reaches 100% when bulk-rated files have left the folder.
- `handleSortByPrediction`: **awaits** `initClipModel()` behind a new `clipProgressSink` (mirrors `extractionProgressSink`) rendering `Downloading CLIP model… N%`, with an abort check after.
- `handleMlWorkerMessage` `'progress'`: card form while a prediction sort owns the element; left **untouched** when a similarity sort owns it; plain text with counts appended otherwise.
- `trainFromHistoricalRatings` now tears the card down on **no** path — `handleSortByPrediction`'s `finally` is the single owner.

**Two fixes beyond the brief, both required for the items' stated properties to hold**
1. 🔴 **The BACKLOG entry's mechanism for item 2 was wrong, and the truth was a correctness bug.** It said the un-awaited `initClipModel()` made the first `extractClipEmbedding` "silently wait" on the model. It does not wait — it returns `null` while `clipWorkerReady` is false. So on a cold sort every historical like/dislike processed before the ~87 MB model loaded trained on a 576-dim vector whose **CLIP half was all zeros**, silently, because a null embedding is a supported degraded mode. Reachable on a fresh session: since P3, folder open no longer pre-warms CLIP. The ordering test fails against the old code as `train(clipReady=false)`. User approved the wider scope.
2. **A third call site would have undone item 1 one phase later.** The ML worker answers `trainHistorical`/`sortByPrediction` with its own progress; those also routed to `updateProgressNotification`, so Cancel would still have died for the tail of training and all of scoring — the item would have looked fixed while its stated property was false.

**Review** (5 reviewers + adversarial pass, no PR): **one** finding, comment-and-dead-line rather than behaviour — the added indeterminate prime before Phase 2 was overwritten before any paint, because `startBackgroundFeatureExtraction`'s head is await-free and reaches `extractionProgressSink` synchronously with a real `cached/total`. Three comments asserted the false premise, including a test comment that restated it while asserting something else. Fixed in `e2b5724` by **deleting** the dead line (not blessing it with a corrected comment), rewriting the rationale for the — still correct — clear removal, and making the clears symmetric. Deleting the cancel-path clear removed the only assertion that a cancelled sort tears its card down, so that assertion moved to where the behaviour actually lives (`handleSortByPrediction`'s cancel test); the reviewer mutation-verified the move. Also folded in the predicate gap the review confirmed reachable with no user action (`requestPredictionScores()` fires by itself when extraction completes with a ready model) — taking a third option over the suggested `|| isComputingHashes`, which would have preserved Cancel at the cost of mislabelling the similarity sort's phase.

**Verification**: unit **597 → 613** (18 files); full E2E **56/56** on every push and on the merge; lint clean apart from one pre-existing `no-shadow` warning (confirmed by stashing); `format:check` clean. Every new test watched fail first.

⭐ **A test that passes against unfixed code is not a test.** The first CLIP-ordering test passed at RED time — a 10 ms fake model load finishes inside the lazy-init block's existing 100 ms settle sleep. Rewritten to 400 ms, which is deterministic rather than a race (both timers are registered before either fires; `setTimeout` preserves delay order).

⭐ **PR #34 already decided this, at one of two sites.** `4ea65c3` (May 8) fixed the identical fire-and-forget `initClipModel()` after review, leaving a comment that describes this exact bug — but `git show 4ea65c3^:media-viewer.js` shows **two** un-awaited calls, at 6306 (inside `handleSortByPrediction`) and 6936; only 6936 was fixed. `637c867` later touched the surviving line but did not introduce it (`- this.initClipModel();` → `+ if (this.enableClipFeatures) this.initClipModel();` — a guard that was already there). So this is a **missed sweep**, not a regression, and the same lesson as PR #56/#66: when fixing a *named pattern*, sweep every call site, not the one in front of you. `4ea65c3`'s own body had even adjudicated the concurrency question this branch re-raised ("concurrent calls are deduped by the IPC layer (cosmetic duplicate progress listener may register)") — a sweep would have inherited a settled answer instead of needing a fresh one.

**Follow-ups**: 3 🟤 `[2026-09-02]` filed (zero-CLIP caching in `startBackgroundFeatureExtraction`; the 30 s CLIP unload timer reaching the same corruption through a second door; no `ml-worker.js` test harness). The first two are the same corruption by other doors and should be fixed together; neither is reachable-worse because of this branch.

### 2026-09-02 — Group G3: Docs & process guardrails 🟤 (+1 🟡) (Cleanup Week #3) — **6/6, MERGED `45a0d9b`**

**Plan**: [docs/archive/plans/2026-09-02_g3-docs-process-guardrails.md](../archive/plans/2026-09-02_g3-docs-process-guardrails.md)
**Spec**: none — brainstormed as a **bounded** change; six independent edits to code and docs that already existed.
**Merge**: `45a0d9b` (`--no-ff` into `main`, **no PR** — reviewed locally by 5 reviewers; branch deleted local + remote)
**Commits**: `4b5950b` (guard + backfill) → `15b6eb7` (4 nits) → `0f2d89c` (review round) → `c726445` (known limitation)

**Summary**: Replaced two chronic manual-checklist failures with automation, cleared the residue they left, and stopped local verification from costing a retry. Closes 10 BACKLOG entries from 6 groups.

**Key changes**:

- **`scripts/check-docs-index.js`** — pre-commit guard in the `check-secrets.js` / `check-e2e-needed.js` house pattern (dependency-free Node, pure functions + thin CLI). Keeps `docs/README.md` honest in **both** directions: every file under `docs/archive/plans/` and `docs/superpowers/specs/` must be linked, and every link must resolve. Reads the **git index**, so it guards the commit rather than the worktree.
- **24-row index backfill**; three stale duplicate plans in `docs/superpowers/plans/` deleted (the directory with them); the dead `video-fullscreen-toggle` rows cleared; `docs/README.md`'s hand-maintained "Last Updated" footer dropped; and the second, unenforced index in `docs/archive/plans/README.md` (drifted to 18 of 60) deleted rather than mirrored.
- **Closeout conventions** — a Closeout-artifacts table derived from the global Task-Completion rule, a live-surface preflight step, and a "don't restate a derived count" convention. The out-of-tree `.claude/TEMPLATES/` half filed as a TODO § Spawned Task.
- **`.vscode/settings.json`** — Markdown format-on-save off at the repo level, plus LF to match `.gitattributes`.
- **`check-e2e-needed.js`** — explicit 64 MB `maxBuffer`, parity with `check-secrets.js`.
- **`vitest.config.js`** — `pool: 'threads'`.

**Verification**: unit **556 → 597 passing / 18 files** (+41 new tests for the guard); E2E **56/56**; guard mutation-tested and negative-tested end-to-end for both failure modes.

**Review**: 5 reviewers + execution-based checks found **2 blocking defects in the guard itself**, both reproduced before fixing and re-verified after (`0f2d89c`).

1. **Partial staging bypassed the guard entirely.** It read the working tree; `git commit` commits the index. Stage a new plan, forget `git add docs/README.md`, and the commit landed an unindexed plan with the guard green — the exact failure it was built to prevent. Sharpened by a rule this repo already had: *stage docs by explicit path, never `git add -A`* makes partial staging the **normal** path here.
2. **The `readdirSync` catch swallowed every errno**, not just the ENOENT its comment justified — silently zeroing the MISSING half while BROKEN kept passing. A verbatim repeat of PR #63's one Minor (`018f0d2`), same invariant, one script later.

**Key decisions**:

- **Path-matching over basename-matching** — a file mentioned via a path it no longer lives at is not indexed; it is a missing row *plus* a dead link. Presence-only, as the source entry specced, could not have prevented the very defect this group cleaned up by hand.
- **Delete the second index, don't backfill it** — mirroring a machine-checked list by hand is guaranteed to lose.
- **`threads` over `fileParallelism: false`** — chosen on measured cost after the flake failed to reproduce (15/15 green), and after finding the entry's suggested `pool: 'forks'` was a no-op because forks *is* v4's default.

**Lessons learned**:

- **A probe that never executes the target returns a pass-shaped answer.** `require()` does not set `require.main`, so probing a CLI guarded by `require.main === module` measured an *empty program* — and "confirmed" a real defect twice on worthless evidence. Same class as the defect under investigation: a check that cannot fail looks exactly like one that passes. Be most suspicious of a probe that agrees with you.
- **Measure an item's premise before executing it.** The vitest entry was wrong in both directions at once — the flake did not reproduce, *and* its proposed remedy did nothing.
- **A guard's scope must match what it guards.** The structure followed the house pattern; the *scope* did not, and that is where it failed open.

**Follow-ups**: BACKLOG 🟤 `### [2026-09-02] From: G3 closeout` (3 items — unclosed-fence parser limitation; no per-check hook bypass forcing `--no-verify`; the derived-count sweep the convention landed without).
...
**Deviations from process**: closeout (`1d8868c`) was written while the branch was still unmerged, on the then-current decision to record state rather than merge; the merge followed the same day (`6305a7a`) and falsified seven live assertions across WEEKLY and this file, reconciled in `bb4db1c` — and this very paragraph was the one site that sweep missed, corrected afterwards. That is the 🟤 `[2026-08-31]` "closeout precedes merge" entry, demonstrated on itself. TODO.md transition N/A (G2 was scheduled BACKLOG → WEEKLY directly). Four 🟤 `[2026-08-31]` items filed across two sections: the un-awaited re-entrant renders (one defect, three sites — the blocker) and the re-entrancy guard re-filed as blocked on it, flagging that `g2-serialization-wip`'s `_isForeignLoadInFlight()` changes a pre-existing guard and needs its own review; plus the one-machine commit pointer and the closeout-precedes-merge ordering. One process slip, self-reported: a test file was edited via a Bash one-liner mid-session, breaking the Edit/Write-only constraint this group is the `dead-rules-audit` trial vehicle for. It was reverted and redone through Edit — which means **the audit record looks clean while the violation still happened**. That is a result for the trial, not a footnote: the `PostToolUse` matcher observes tool calls, so a Bash write is structurally invisible to it, and a scorecard reading "compliant" cannot distinguish that from real compliance — the same failure mode the trial exists to catch, one level up.

### 2026-08-30 — Group G4: Adopt-queue trial batch + cadence decision 🟤 (+1 🟡 folded) — **3/3, COMPLETE** (third item read out 2026-09-02, PR #67)
...
✅ **Status: 3 of 3 items complete. MERGED 2026-08-30 into `main` locally** (merge `bf58c01`, `--no-ff`; branch `g4-adopt-queue-trial` deleted remote + local) — **no PR** by task-brief instruction. The third item, the `dead-rules-audit` scorecard read-out, is **legitimately pending its vehicle**: the plugin scores `Edit`/`Write` calls across the G2 + G3 sessions and reads out at **G6 on Fri Sep 4**. This entry is amended then, not replaced. ✅ **AMENDED 2026-09-02 (G6, PR #67)** — the read-out landed two days ahead of the Fri Sep 4 slot, on the G6 branch rather than a G4 one, and the outcome is **`drop`** as a compliance tool (kept installed as a relevance heat-map). The precondition this status line worried about held: the vehicle produced ample data (13 sessions, 2,134 judgements). What failed was the scoring — `judge()` never separates a rule's prescribed tokens from its prohibited ones, so compliant edits score as violations and 10 of 43 rules were flagged `⚠ promote→hook`, led by the two tournament rules G2 had just shipped and enforced. No re-trial: the defect is in the scoring function, not the vehicle. Full read-out in [REVIEW-QUEUE.md](REVIEW-QUEUE.md) § 5; see also the G6 entry above.

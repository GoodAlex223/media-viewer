...
🟡 **Status: User-side 24k smoke PASSED 2026-07-20 — PR #64 OPEN** (branch `feature/g1-ai-sort-startup-ux`, not yet merged; 21 commits above `main`, merge-base `dc43736`). **User-side 24k manual smoke PASSED 2026-07-20** on the user's real 20,929-file folder — all 5 gating checks (immediate determinate card, no redundant re-extraction on a warm cache, Cancel actually aborts load+extraction+sort leaving the list unsorted, identical post-tournament-exit behavior, reported load-time improvement). **434→471 unit** / **full E2E 52/52** / lint clean. **Two post-review incidents, both real and both fixed in-branch**:

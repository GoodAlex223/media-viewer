...
- **[BACKLOG.md](BACKLOG.md)** — `### [2026-09-02] From: Weekly Reviews (2026-09-02 run) (3 items)`: two adopt-derived, each naming its trial vehicle, plus one non-adopt item the closeout's own link check surfaced (27 broken relative links in this file's March–May entries). The `dead-rules-audit` trial entry flipped to `[x]` with its outcome.
...
- 🔴 **This branch broke an existing written convention three times, and the convention already named both the failure and the cure.** [planning/README.md](README.md) L58–66 says: _"Do not restate a count or range that is derived from a list elsewhere — reference the list open-endedly instead… The fix that held was deleting the value, not refreshing it,"_ with a review-time tell — _"verify whether the same commit extended the source the fix just described."_ All three PR-review findings of this shape are instances: DONE.md's `(2 items)` after `a4d72a4` added a third; the run-card's Outputs list assuming adopts are the only source; and the positive control's expected count of "three", where the same commit added two more matching mentions to DONE.md. The reviewer found the third by applying that documented tell verbatim. Two things generalize past this PR. **A convention is not a control** — this one was written, correct, specific, and violated three times in the branch that came after it, which is the same "advisory rules are not self-executing" result § 5's visual-verification read-out produced from the opposite direction. And **the cure for a derived count is deletion, not accuracy**: the fix that held here replaced the expected count with an expected _property_ (`> 0`), because a calibrated count fails on a correct tree the moment prose changes — an expected value that goes stale is a dead check, which is what the sweep learning above is about.

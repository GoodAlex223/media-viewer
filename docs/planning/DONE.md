...
- **Minors**: folder scans sorted by name in both `main.js` and `ml-training.js` (`trainBatch` shuffles an index array *derived* from scan order, so the fingerprint seed alone did not make weights reproducible — a real hole in § 5.4's "a cache hit is verifiable" claim); the duplicate `trainComplete` training toast removed (D6 made the indicator the once-per-sort report, and `showNotification` evicts the oldest of five — the sort card **is** a notification element); the never-read `_mlWorkerVersions.featureDim` dropped; and this entry's own commit range re-notated as "through ... inclusive" (`bcd6fa9..e6a37f9` counts 17, not the 18 stated — the count was right, the notation falsifiable in one command).

...
- **[BACKLOG.md](BACKLOG.md)** — `### [2026-09-02] From: Weekly Reviews (2026-09-02 run) (2 items)`, one per adopt, each naming its trial vehicle; the `dead-rules-audit` trial entry flipped to `[x]` with its outcome.
...
**Follow-up tasks**: BACKLOG 🟤 `[2026-09-02] From: Weekly Reviews (2026-09-02 run)` (2 adopt-trial entries); TODO § Spawned Tasks (1 propagation, plus the 2026-08-27 one still unapplied). Open for the next planning conversation: whether to extend § 5's trial-log discipline to `propagate` verdicts, and whether a "runner-up" that is really a non-candidate is worth parking (three consecutive runs where §2's fresh check beat the parked list).

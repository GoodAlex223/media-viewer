...
### [2026-06-07] Group A (JXL viewer) implementation-review follow-ups (4 items)

**Origin**: Per-task + final whole-branch code review of `feature/jxl-viewer-support` during subagent-driven implementation. The one threshold finding (the `ensureJxlWorker` hang on init failure, ~85/100) was fixed before merge in `84bf62b`. The items below are sub-threshold / pre-existing / out-of-scope and tracked as follow-ups.

...
**Last Updated**: 2026-05-03 <!-- feature/clip-sort-followups manual-testing finding -->
...
- [ ] 🔴 **CLIP background extraction silently does not fire on folder load** — Repro: enable CLIP in Settings (F1, default ON anyway), open a folder that has never had `.feature_cache.json` (e.g., a fresh folder), wait 60+ seconds. Observed: no `.feature_cache.json` is created, no progress notification appears, no CLIP-model-load notification, no console errors. Subsequent click on Sort > CLIP throws `"Only 0 files have CLIP embeddings. Wait for background extraction to complete, then retry."` from `handleSortBySimilarity` ([media-viewer.js:4170](media-viewer.js#L4170)). Hash sorts (MST/VPTree) work normally on the same folder — they create `.hash_cache.json` and complete. Suggests `startBackgroundFeatureExtraction` is either (a) not being called on folder load, (b) called but early-returning before any CLIP work begins, or (c) silently failing in a way that swallows logs and notifications. Investigation needed: trace the call chain from `loadFolder` → `startBackgroundFeatureExtraction` → CLIP queue path; add diagnostic logging at each guard. Affected: `media-viewer.js` (renderer extraction orchestration), possibly `main.js` (CLIP IPC). NOT introduced by feature/clip-sort-followups — the branch's diffs are all downstream of having CLIP vectors. Likely regression from a prior change OR pre-existing behavior the user only noticed now. High priority — blocks all CLIP-based features end-to-end.
- [ ] **Add UX-visible "extraction starting" notification** — Even if extraction is firing, the absence of any visible "Loading CLIP model..." or "Extracting features: 0/63" feedback for the first ~1-30 seconds creates a "did anything happen?" moment that prompted the bug above to be reported. Adding a "Starting feature extraction..." toast immediately on folder load (before any per-file progress) would surface failure modes faster and improve perceived responsiveness. Cosmetic but high UX value once the bug above is fixed.

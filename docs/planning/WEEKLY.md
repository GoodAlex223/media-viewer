...
- **🟤 tail remains after this week**: CW batches burn ~16 SP of auto-generated debt, but the 🟤 section holds substantially more (plus older XS items from March–April). The stale-checkbox sweep (CW-3) will produce an accurate pending-SP recount; expect the next Cleanup Week around **early July 2026** (~3-week cadence) unless the recount says otherwise. **Post-CW-3 recount (2026-06-16):** **153 unchecked 🟤** after CW-3 cleared **11 🟤 entries** total — 4 via the sweep (CLIP similarity sorting, CLIP unload, logger double-init, regression-checker FullscreenManager) + 7 at closeout (the CW-3 task entries: sweep, docker cruft, nul line, kickoff doc-drift, waitForTimeout dup-tag, + 2 already-done no-ops: MANUAL markers, backlog-structure inventory). Net **155 unchecked 🟤** including the 2 CW-3 closeout follow-ups filed (SHA-verification convention, continue-the-sweep). The tail is still large → the ~3-week Cleanup cadence holds. (Count is checkbox lines in the 🟤 section, used as an SP proxy — the items are overwhelmingly XS.)

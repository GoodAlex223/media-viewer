...
**Created**: 2026-09-03
**Spillover**: delivered through **2026-09-24**, 13 days past its Friday — G1 and G4 merged 09-12, G2 09-13, G3 09-21, and G5 (Weekly Reviews) ran 09-24. ⚠️ **Recorded late** — by G5 on 2026-09-24, not "as soon as the slip is known", as the July lesson in the Previous Week Summary asks. The slip was plain by 09-12, and eight commits edited this file after the Friday without adding this line, four of them group closeouts (`68a7a91` and `3198786` on 09-12, `6423232` on 09-13, `794b146` on 09-21). Its consequence is the one that lesson warns about: **Cleanup Week #4** (Sep 21–25, confirmed 2026-09-03) fell due during the spill and has no plan yet.
...
**Cleanup Week?**: **No.** Cleanup Week #3 ended 2026-09-04 (all groups merged by Sep 3); the ~3-week cadence puts #4 at ~September 21–25. ⚠️ The **🟤-size trigger has already re-fired**: measured 2026-09-03, **47 unchecked 🟤 items** sit in sections dated since July 10 (~~five~~ **four** of them a closeout miss — corrected 2026-09-24, see Notes), and Cleanup Week #3 _filed_ more 🟤 than it _closed_. Surfaced per the rule; recommendation is to hold the cadence rather than run two Cleanup Weeks back-to-back, since the 🔵 [2026-08-28] intake has waited since August and the ≥50% floor is a hard rule. Confirmed by the user 2026-09-03 (Notes → rulings).
...
- **TODO § Spawned Tasks — all three rows applied by G5 on 2026-09-24**, by user ruling at G5's brainstorm, together with a fourth propagated in the same run. The `realness` rating axis (2026-08-27), the negative-finding evidence rule (2026-09-02) and the plan-template closeout block (2026-09-02) were all edited live in `~/.claude`; `claude-code-universal-config` receives them through its own one-way sync. Their states now live in REVIEW-QUEUE.md § 4's outbound outcome log — the convention the burn-down ruling below asked for.
...
- **REVIEW-QUEUE §4 inbound** (`/wayfinder`, Jenkins, the design-video sub-batch) — third consecutive check at G5; per the 2026-09-02 note, if still unchanged upstream, **ask the origin repo directly** instead of re-checking a fourth time. — ✅ **done by G5, 2026-09-24**: still unchanged upstream, so the origin repo's owner — the user — was asked in-session and ruled per item: `/wayfinder` folded into `mattpocock/skills`, Jenkins dropped, the design-video batch parked event-driven (REVIEW-QUEUE § 4, inbound row).
...
> ⚠️ **G1's three task checkboxes were flipped by G4, not by G1's own closeout (2026-09-12).**
> They still read `- [ ]` after PR #68 merged as `bfcc881`, while this group's Summary-Table row and
> all three Daily-Schedule rows already read done — the *same* closeout miss G4's 0-SP housekeeping
> item exists to repair, recurring one group later and in the same file. Verified before flipping,
> per deliverable rather than per claim: the spec exists at
> [2026-09-10-ml-training-pipeline-design.md](../superpowers/specs/2026-09-10-ml-training-pipeline-design.md);
> `ml-training.js` exists and is the module in CLAUDE.md § Architecture L47; CLAUDE.md carries the
> `resetMlModel()` bullet; the deferred-refresh 🟤 cluster carries its rulings; and
> `tests/e2e/ml-retrain-skip.test.js` holds the source-folder-switch E2E the second task promised.
> This is now the **fifth** instance of "a late fact invalidating a doc written earlier" on this
> branch line — see 🟤 `### [2026-08-31]` item 2, which proposes the post-merge reconcile script.
...
- [x] _0 SP housekeeping — closeout miss found at planning_: **check off the five PR #65 entries G2 shipped** (BACKLOG `### [2026-07-21]`: `reconcileWithFiles` untracked prune; failed special-restore wedge; document the `exitTournamentMode` invariant; empty-state keydown guard; CLAUDE.md line-191 split) with `6305a7a` + the G2 commits as evidence — all five read `- [ ]` today although WEEKLY, DONE and the archived plan record them shipped. Do it in this branch's first commit so the evidence is one `git log` away. ⚠️ **Corrected during G4 implementation — "G2 shipped five" was true of four.** Items 1–4 verified against `68a7a91` and flipped with per-item code evidence. Item 5 (the CLAUDE.md line-191 split) was **never performed**: its own text deferred it to "the next CLAUDE.md quarterly audit", and no G2 commit split it. It is flipped on a different ground — the bullet went 964→595 chars as a side effect of G2's content corrections (`3eff3eb`, `b352360`), so the stated condition is gone — with the live successor condition (longest bullet now 1501 chars; file 211 lines vs a 205 soft cap) handed to the open path-scoped-rules entry rather than re-filed.
- [x] **Close the remaining zero-CLIP doors** — (a) clear `clipUnloadTimer` wherever the model is _demanded_, not only where extraction starts: in `initClipModel()` and in `handleSortByPrediction`'s Phase 1 even when `clipWorkerReady` is already true; unit test: a pending timer cannot flip `clipWorkerReady` during a sort (arm → sort → advance fake timers → assert ready). (b) decide the `success:false` door: either refuse to cache CLIP-less entries while `enableClipFeatures` is on (extraction stops, user told), or record on the entry that the self-healing `!hasClip` re-extract filter is the accepted answer and the "un-awaited caller" mechanism was not reproduced. `media-viewer.js` (2) — 🟤 [2026-09-02] G5 closeout ×2
- [x] **`ml-worker.js` unit harness** — `importScripts` shim + `globalThis.self` stub (the `sorting-worker.js` house pattern, adapted); pin the four `updateProgress` sites, the abort-flag protocol, and the `trainHistorical` / `scoreComplete` / `sortComplete` reply shapes; drive via `self.onmessage`. Lands before G1 changes `trainHistorical`. `tests/ml-worker.test.js` (new), `ml-worker.js` (conditional CJS export only if unavoidable) (3) — 🟤 [2026-09-02] G5 closeout
...
> ⚠️ **Held 2026-09-24, not Friday Sep 11 — two premises above were corrected at execution.** The window was **22 days**, not ≈9, so `pass (no new candidate)` was no longer the presumptive answer for §1. And the § 5 cost figures did not need the Installed tab: `claude plugin details` prints Claude Code's own "Projected token cost", so no cost figure came from file sizes or waited on the UI — though the tab's own **Not used recently** reading, which only the UI has, was never taken (run-card deviation 9). Run-card: [`2026-09-24-weekly-reviews-run.md`](../superpowers/specs/2026-09-24-weekly-reviews-run.md).

- [x] **§ 5 read-out — installed-plugin context-cost & disuse audit** (0 SP, carried from the 2026-09-02 `adopt`): record every installed plugin's `Context cost` / `Last used` / **Not used recently** as a table in § 5; disable or uninstall anything both expensive and unused; **revisit `dead-rules-audit`'s keep-installed disposition on the spot** if its cost is real. — 🟤 [2026-09-02] Weekly Reviews item 1 — ✅ **read out `keep`, 2026-09-24**: eight claude.ai-synced plugins disabled user-wide, `pr-review-toolkit` disabled at project scope, `hookify` and the `context7` plugin disabled for this repo, `dead-rules-audit` **uninstalled** (its cost was tiny, but the heat-map it was kept for had no reads), and four dangling settings entries removed — every disposition user-approved; table in REVIEW-QUEUE § 5. ⚠️ Delivered **without** the **Not used recently** reading this box names: only the `/plugin` tab shows it, and it was never read. The table now records the column as `not read` (run-card deviation 9), and the Recurring read-out policy carries it forward.
- [x] **Plugins (2 SP)** — official store + wider internet, each with `source:`; roundups are leads to check against the primary roster, never a source for a verdict (two consecutive runs asserted nonexistent official plugins). — ✅ §1a `claude-security` **adopt** (the one trial slot), §1b `dead-end-registry` **pass** (measured against this repo's transcripts). ⚠️ **The parenthetical above is wrong** and is corrected in REVIEW-QUEUE §1: all three "nonexistent" official plugins are in the catalog — the docs page those runs checked is not the roster.
- [x] **Claude best-practices (1 SP)** — hybrid: fresh check vs. the parked list (third consecutive run the fresh check won; if a fourth, drop non-candidates from Next-up). — ✅ version the project's `.claude/settings.json` → **adopt** (known shape, 🟤); the fresh check won a fourth time, so the non-candidates were dropped from Next-up.
- [x] **Non-Claude AI best-practices (1 SP)** — ✅ sandbox-by-default agent execution → **pass** (the Claude Code sandbox does not support native Windows).
- [x] **Cross-project propagation (1 SP)** — outbound: scan this week's merges (G1–G4) with the high bar; likely candidates are the two _planning-time_ lessons this plan produced (a BACKLOG entry's mechanism is a hypothesis to re-measure; a closeout must flip the entries it closes) only if absent globally — grep the target, never assume. Inbound: third check; if unchanged, **ask the origin repo directly**. Record the applied/unapplied state of the three § Spawned Tasks rows and the user's ruling on the `propagate` burn-down question (open question below); if the ruling is "extend § 5", add the outcome-row convention to § 4 Conventions in this run. — ✅ **outbound**: the closeout lesson was propagated **and applied live**, together with the three earlier rows, by user ruling; the outcome-row convention and an outbound outcome log are in § 4. The mechanism-hypothesis lesson stays parked with fresh evidence. **Inbound**: every sibling row naming this repo was ruled per item by the user — the "ask directly" step — including two that the 2026-09-02 sweep had missed.
...
- **Closeout miss found at planning**: BACKLOG `### [2026-07-21] PR #65 review follow-ups` still shows all nine entries unchecked, although G2 shipped ~~five~~ **four** of them (`6305a7a`; WEEKLY G2 row, DONE 2026-08-31 and the archived plan all record it). The G2 closeout commit (`1d8868c`) filed two new entries and did not flip the ~~five~~ **four** it closed. _(Corrected 2026-09-24, in PR #72's review: G4's housekeeping item above established four — the fifth, the CLAUDE.md line-191 split, was never performed. This note was not swept when G4 made that correction, and it re-seeded "five" into three G5 documents.)_ Scheduled as G4 item 0 rather than fixed silently here (unattended run; BACKLOG edits beyond the plan are user-gated). The general fix is the "closeout is written before the merge" 🟤 [2026-08-31] item 2 — this is its fifth instance.
...
- **Open questions — user rulings 2026-09-03** ("Agree with assumptions"; the bold assumptions below are now decisions):
    ...
    3. `propagate` burn-down (REVIEW-QUEUE § 4 asked this conversation): extend § 5's outcome-row discipline to `propagate` verdicts, or accept § Spawned Tasks as user-maintained and stop counting "filed" as done? **Assumed: extend § 5** — ✅ confirmed: G5 adds the outcome-row convention to REVIEW-QUEUE § 4 Conventions in its run. Related: whether the three spawned two-trees edits get applied this week (user-side) — ✅ **resolved 2026-09-24 by G5**: applied live in `~/.claude` by user ruling, with a fourth from the same run; see REVIEW-QUEUE § 4's outbound outcome log.

...
- [ ] **Smarter timing for background feature extraction (don't always start on folder open)** — 3 SP, 🟠 IMPORTANT — ⏳ **IMPLEMENTED 2026-06-25** (branch `feature/extraction-timing`; chose **pure lazy**, no settings toggle; 4 tasks, reviews Approved, opus final "Ready to merge: Yes", 381 unit). Box stays unchecked — **manual 24k smoke + merge pending**. See [DONE.md](DONE.md) 2026-06-25.
...
- [x] Decide extraction-timing strategy (lazy / threshold / toggle / idle) + implement gate (3 SP) — chose **pure lazy** (no settings toggle needed); implemented + reviewed on branch `feature/extraction-timing`; ⏳ 24k smoke + merge pending before final checkoff

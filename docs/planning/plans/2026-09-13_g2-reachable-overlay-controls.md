...
- 2026-09-13 — Task 2: Step 3's "Expected: 7 failed" was wrong. Actual RED run was 5 failed / 2 passed. "compare: leaving compare mode clears the side badges" passing today is correct and not a defect — it guards a hazard Task 5 introduces (a container-anchored badge outliving the mode switch), where today's wrapper teardown already clears it for free; kept verbatim with a comment recording this. "compare: the prediction badge stays inside the viewport" passed for a real reason: `getBoundingClientRect()`-based viewport checks can't observe an ancestor's `overflow: hidden` clip, so the assertion as planned had zero coverage of the bug it names and could never go RED→GREEN. Fixed by adding an `isUnclipped(page, selector)` helper (DOM walk to the nearest clipping ancestor, checked in one `page.evaluate`) and a containment assertion alongside the existing checks — now correctly RED (`left badge is clipped by an ancestor`, failing on the new assertion, confirmed not a typo). Suite is 6 failed / 1 passed as committed.

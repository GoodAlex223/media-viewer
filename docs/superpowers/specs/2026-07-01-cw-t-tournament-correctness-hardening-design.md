...
- **Undo semantics must remain equivalent** to today (session-only, capped at 100; undo restores the exact prior pair/state). The inverse-delta path must be behaviorally equivalent to the snapshot path for the pick-streak hot path. `engine.files` rewind across a mid-tournament `removeFile` **is preserved** (the existing regression test [tournament-engine.test.js:109-132](../../tests/tournament-engine.test.js) MUST stay green — see C2 "removeFile interaction"). The one narrow, documented nuance: a file removed mid-tournament reappears in `engine.files`/tier count on undo but is not re-inserted into the strategy's pairing (a delta can't) — see C2.
...
**`removeFile` interaction (the key correctness constraint).** Today, `engine.undo()` ([tournament-engine.js:364](../../tournament-engine.js)) restores the full `strategyStateSnapshot` **and** a per-pick `filesSnapshot`. The `filesSnapshot` is what lets undoing a pick recorded *before* a mid-tournament `removeFile` rewind `engine.files` (so `getTierBreakdown`/`handleApply` don't under-count). This is a **tested, intentional contract** ([tournament-engine.test.js:109-132](../../tests/tournament-engine.test.js)).

Design decision — **keep `filesSnapshot`; delta replaces only the strategy snapshot**:
- **`filesSnapshot` is captured on EVERY history entry, unchanged** (delta *and* boundary-snapshot entries). It is a cheap `[...this.files]` array-of-refs copy (~24k pointers, sub-millisecond) — ~50-100× cheaper than the eliminated `strategy.serialize()`, which allocated ~36k small arrays via `winCounts.entries()` + `roundQueue.map()`. So the per-pick win is preserved while the `engine.files` rewind contract stays intact.
- `undo()` of a **delta** entry: apply the inverse-delta to the strategy **and** restore `this.files` from `filesSnapshot`. `undo()` of a **snapshot** entry: unchanged (`deserialize` + restore `filesSnapshot`).
- **Narrow documented nuance:** undoing back *past* a mid-tournament `removeFile` restores `engine.files` (the file reappears, tier 0 via `getTier`'s `?? 0`) but does **not** re-insert it into the strategy's `winCounts`/`roundQueue`, so it is not re-paired on continued play. This matches the existing test's assertions (files + tierBreakdown) and is acceptable: special-moved files are recovered via the dedicated renderer special-undo branch ([handleTournamentUndo:4539](../../media-viewer.js) `actionType==='special'`, re-adds to `engine.files` independent of `engine.undo()`); an externally-deleted file (the `-1` branch) should not re-enter play. Full strategy resurrection would require snapshotting every pick — the very cost being removed.
- Pin with tests: (a) the existing removeFile+undo test stays green (regression guard); (b) undo a streak of N picks ≡ old full-snapshot behavior; (c) draw-`win`/draw-`lose` undo; (d) boundary snapshot undo; (e) undo at the 100-cap.

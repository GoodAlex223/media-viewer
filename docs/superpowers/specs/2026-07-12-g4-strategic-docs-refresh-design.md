...
| **D10** | § 4 and D4 name the mechanism the *"weekly-planning staleness check"*, and the drafted docs asserted "run as part of every **weekly** planning session". The measured gap between the last two review runs was **7.5 weeks**, and the user confirmed the cause is availability, not slippage — the real cadence is ~monthly. The G5 run-card had already flagged this (line 217). The refresh therefore shipped a cadence claim its own repo evidence contradicted — the same defect class as D6, committed in the same session | **Make the wording cadence-neutral** (user-approved 2026-08-27, after the initial commits): "planning-session staleness check"; "run as part of every planning session, whatever its cadence"; "work flows each planning session". True under any cadence, so it cannot go stale if the rhythm changes again. The **2-month backstop is unchanged** — the contradiction trigger is the load-bearing half and is cadence-independent; at a ~monthly cadence the backstop's worst-case detection latency is ~3.7 months rather than the ~2.2 the "weekly" wording implied. Renaming the `WEEKLY.md` artifact itself was considered and **deferred** as its own project (touches BACKLOG/DONE/TODO/README/CLAUDE.md/REVIEW-QUEUE + the "weekly SP quota" language). `WEEKLY.md`, "Weekly Reviews" and "current week's plan" remain as proper nouns |

### 9.2b Post-review amendments (code review, 2026-08-27)

A review of the branch found five issues sharing one root cause: **D10 was decided late in the branch and never swept back over docs written earlier in the same branch** — the exact failure mode the global CLAUDE.md propagation check describes ("a late correction routinely reaches the task journal and stops there, because the live-doc edit was made *earlier* in the task and nothing revisits it"), committed in the same session whose D10 commit message quoted that rule. All five verified against primary evidence before fixing.

| # | Amendment | Detail |
|---|-----------|--------|
| **D11** | **`PROJECT.md` is in scope** — the spec's file list omitted it | `PROJECT.md` asserted the *exact* claims this refresh exists to kill: `Testing \| Manual (no automated tests)`, `# Run tests (not configured)`, `~6100+ lines`, `No automated tests: Manual testing only` — plus `No linting configured` / `No pre-commit hooks configured`, which the review did not list but are equally false. Global CLAUDE.md § Task Completion names `PROJECT.md` explicitly as a live surface to check, and Knowledge Sources reads it at **step 2, before `docs/`** — so the stale copy sat in the *higher-priority* source while the branch fixed only the lower one. The driving BACKLOG entry (`BACKLOG.md:202`) cites those sentences verbatim as the reason for the work. This is the [[derived-checklist-vs-governing-rule]] failure: the spec's file list was followed *instead of* the rule it implements, so its omission read as completeness |
| **D12** | **Never enumerate § 9's range in another document.** The same defect fired **three times in a row**: `baed3ba` wrote "D6–D9" while D10 was pending; `02998ec` fixed that to "D6–D10" *and added D11 in the same commit*; restating it as "D6–D11" would only reset the clock again. A value derived from a living table is guaranteed to go stale, and the fix is to **remove the derived value**, not refresh it | References to § 9 are now open-ended — *"§ 9 records the 2026-08-27 re-verification (**D6 onward**)"* and *"amendments recorded as **D6 onward** in spec § 9"* — so there is nothing left to keep in sync. The historical count in the D10-sweep note below ("the four docs written before D10") is a completed-action tally, not derived from the table, and cannot drift. **Review-time tell** (from the reviewer): when checking a fix, verify whether the *same commit* extended the source the fix just described |

**D10 sweep completed** (the four docs written before D10 and left contradicting it): `docs/README.md:33` ("install the **weekly** staleness check" → planning-session), `docs/README.md:167` ("§ 9 amendment **D6–D9**" → D6–D10, plus a pointer that D10 supersedes D4's "weekly" wording), the plan's superseded banner ("**four** amendments … D6–D9" → five … D6–D10), and the plan's prescribed future Archived-Plans row at `:424`, which would otherwise have **re-injected the retracted "weekly staleness rule" wording into a live doc at closeout**. Also refreshed `docs/README.md`'s chronically-stale footer (2026-07-20 → 2026-08-27), an already-filed open defect (`BACKLOG.md:355`) whose own prediction this branch was in the act of fulfilling, and added the missing G4 row to `docs/planning/plans/README.md` § Current Plans.
